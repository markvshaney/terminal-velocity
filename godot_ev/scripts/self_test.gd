...
func _initialize() -> void:
	...
	if frame_ok != FRAME_COUNT:
		...
		quit(1)
		return
	print("GODOT SELFTEST OK systems=%d ships=%d %sFrames=%d soundsLoaded=%d pictsLoaded=%d prefScreen=original-ev-classic-observed prefsScreenshot=%s movementLog=deterministic data=native_ev/data/universe.json" % [systems.size(), ship_defs.size(), player_check_id, frame_ok, loaded_sounds, loaded_picts, prefs_screenshot])
	quit(0)
