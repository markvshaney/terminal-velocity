...
The implementation should use a tiered fast path. On clean-idle startup, the preflight only needs `git status --short --branch`, active TV Kanban task/worker state, known runner stop/lock files, structured start-resume JSON, and target-worker capability for required profile skills/tools; if these are clean and no blocked handoff is present, autostart may dispatch exactly one task and verify a heartbeat. Escalate only on concrete triggers: dirty worktree → dirty-handoff classifier; `push_ready` or stale `review_required_process_bug` → integration-owner recovery/normalization; lock/owner ambiguity → full topology inventory; capability mismatch → tooling/profile report; unknown blocked card → inspect comments/events/closeout packet. Stable ledger semantics, speed policy, verifier maps, and source-label rules are not re-derived on every start unless their files changed or a prior run reported staleness. The machine result consumed by watchdogs should stay compact: `safe_to_start`, `recommended_action`, `explicit_gate`, `live_owner`, `dirty_paths`, `blocked_handoffs`, and `heartbeat_or_task_id`.
...
The integration owner performs final status/diff review, runs required checkpoint verification, creates or validates the local checkpoint commit when recovering a stale worker handoff, pushes normal non-force bundles, fetches, verifies local `HEAD == origin/main`, and records the pushed checkpoint. Integration is event-triggered: publish as infrequently as possible while preventing worker blockage, stale coordination, context/reset loss, or inspection-expensive local divergence. Do not push by maximum-frequency cadence, per-commit habit, or arbitrary commit-count batching. Bundle-size triggers are integration-owner triggers, not worker batching permission: a candidate dirty or committed bundle crossing a soft trigger — more than 8 touched files, more than 600 inserted+deleted diff lines, or mixed process-policy and game/model/data changes — must be checkpointed/published or explicitly split before any successor shared-worktree worker starts. A candidate crossing a hard trigger — more than 15 touched files or more than 1000 inserted+deleted diff lines — is not automatically publishable; the integrator must split it or record an explicit reason that the large bundle is mechanically coupled and safer to verify atomically. Mechanically coupled extractor + generated manifest + model + tests changes may exceed the soft trigger when provenance and verifier evidence remain coherent. The deterministic publish preflight is `python3 tools/tv_integration_lane.py --dry-run --blocked-report-target 'telegram:Loki GameTV'`; it must report no active worker, no unsafe dirty worktree, no branch-behind state, only safe TV paths, bundle-size status, `git diff --check`, and committed-diff secret scan before an LLM review may return `publish`. Actual push uses `python3 tools/tv_integration_lane.py --push --llm-approved --post-push-report-target 'telegram:Loki GameTV' --blocked-report-target 'telegram:Loki GameTV'` only after that exact-bundle review. If the integration owner blocks runner progress or causes the runner to stop, it sends the immediate why-blocked report to Loki GameTV; cron/reporting watchdogs are not the primary path. One clean commit may be pushed immediately if it unblocks another lane; several adjacent commits may remain local when no one is blocked and the stack remains coherent and easy to inspect.
...
- **Batch adjacent increments only at the integration-owner/direct-runner surface.** Shared-worktree workers complete one smallest coherent verified increment, emit an `increment_handoff`/`push_ready` packet, and stop before enlarging the dirty working set. The integration owner may batch adjacent handoffs/increments when they share lane/subsystem, source-basis family, verifier surface, and safe ownership; it also owns bundle-size trigger enforcement, checkpoint commit selection, normal push, and successor seeding. Stop at gates, failures, subsystem switches, risky original-runtime steps, checkpoint-policy triggers, bundle-size triggers, or cap/handoff boundaries.
