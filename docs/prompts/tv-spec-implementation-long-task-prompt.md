...
2. Fast-start skim order:
   ...
   2. run `python3 tools/check_tv_runner_topology.py --startup-owner continuous_kanban_runner` before a standalone continuous runner starts implementation work; it derives runtime truth from live surfaces first, then reconciles ledger intent. If it reports `active_owner_conflict` or another conflict type, record the conflict and stop instead of dispatching work. If it reports only `ledger_projection_stale` and/or `ledger_historical_owner_mismatch`, treat `ledger_reconciliation_actions` such as `normalize_ledger_projection` as the follow-up instead of treating the ledger as live-owner truth;
...
- Do not create a `review_required` / `human review` gate for verified safe-local TV code/data/docs changes. If checkpoint policy needs remote publication from a non-integrator worker, record `push_ready` instead of pushing or blocking on credentials. The autonomous integration owner resolves `push_ready` by reviewing, pushing normally, fetching, verifying `HEAD == origin/main`, and recording the checkpoint without waiting for human intervention. If a previous ledger event says `review_required`, re-check live state and resolve it locally when the changes are verified and within TV scope.
