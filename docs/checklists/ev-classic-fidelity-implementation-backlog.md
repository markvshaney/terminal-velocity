...
- [ ] Per-government legal/reputation and AI behavior model
  ...
  - Implementation: Godot now loads `native_ev/data/governments.json` and `native_ev/data/reputation.json`, persists `reputation_scores` and `legal_records` in TV pilot files, exposes current government/legal status on the HUD and Player Info overlay, provides `./run_godot.sh tv-legal-status-log` / `RunGodot.ps1 -LegalStatusLog` as a deterministic legal-threshold contract, and gates outfitter/shipyard purchase actions behind the same scaffold with a visible refusal message plus `./run_godot.sh tv-legal-service-gate-log` / `RunGodot.ps1 -LegalServiceGateLog`. The NPC scanner contacts and target-lock text now surface a Classic Resource Bible-backed CrimeTol-style legal patrol hostile posture; `./run_godot.sh tv-legal-patrol-posture-log` / `RunGodot.ps1 -LegalPatrolPostureLog` verifies the patrol warning without executing combat. Mission availability now honors `requirements.legalMin`, `requirements.legalMax`, and `requirements.reputationMin`, exposes visible blocked reasons on the Mission Computer, and `./run_godot.sh tv-mission-legal-eligibility-log` / `RunGodot.ps1 -MissionLegalEligibilityLog` verifies a clean legal contract becomes unavailable with a visible blocked reason under bad legal standing, matching the Classic `AvailRecord` resource concept. With explicit approval to infer now/correct later, Godot also applies helper-labeled consequences: firing on hostile legal patrols applies `destroy_patrol` legal/reputation deltas without real combat execution (`tv-legal-consequence-log`) as a Classic govt penalty scaffold; landed `C` can buy reputation-gated clemency (`tv-legal-clemency-log`) as a still-inferred bribe/clearing placeholder; and landing with contraband applies a government fine/confiscation scan from `governments.json`/`reputation.json` (`tv-contraband-scan-log`) as a Classic smuggling-scan semantics scaffold with TV-tuned fines. The galaxy map selected-system panel shows a Terminal Velocity legal risk line next to service availability, and the Commodity Exchange selected-row helper now surfaces whether the selected cargo is contraband under the current government plus fine/bribe policy; `./run_godot.sh tv-contraband-risk-log` / `RunGodot.ps1 -ContrabandRiskLog` verifies the Federation/equipment risk line under `sourceLabel=terminal-velocity-classic-resource-smuggling-risk-surface`. The player-facing warning explicitly says exact Classic thresholds/behaviors are unconfirmed; manual-backed logs use `classic_runtime_*_pending` oracle labels while landed clemency remains an inferred placeholder.
...
- [x] Ship/outfit comparison and upgrade consequence surfaces
  ...
  - Implementation: Shipyard listings now show immediate deltas against the current hull for cargo, hull, max speed, and turning; outfitter listings now show concise numeric effect summaries for cargo, hull, and fuel upgrades. Existing Source PICT shipyard art remains visible on the selected listing. Godot also has a deterministic `tv-outfitter-shipyard-log` scenario that routes to Sol/Earth, buys a Cargo Pod and Laser Cannon, injects test-only purchase capital, buys the Light Freighter listing, and reports purchase/cargo/art boolean fields under `sourceLabel=terminal-velocity-outfitter-shipyard-scaffold`.
  - Verification: `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_landing_panels_are_actionable -v`; `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_outfitter_shipyard_progression_log_contract -v`; `./run_godot.sh self-test`; `./run_godot.sh tv-outfitter-shipyard-log`.
...
- [x] In-game learning/documentation surfaces for EV-style strategy
  ...
  - Implementation: Godot now has an F10 help overlay that explains currently wired keys, map route planning, the `G` mission-route helper, active-mission destination markers on the galaxy map, mission/free cargo reservation, landing tabs, buy/accept controls, recent message feedback, player-info/inventory overlay, map service summaries for selected systems, shipyard/outfitter comparison surfaces, refuel, and F6 pilot save/resume. The overlay explicitly labels itself `Terminal Velocity helper/scaffold — not an EV Classic fidelity claim`.
  - Verification: `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_landing_panels_are_actionable -v`; `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_player_inventory_overlay_contract -v`; `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_map_service_summary_contract -v`; `python3 -m unittest native_ev.tests.test_model.NativeEvModelTests.test_godot_map_mission_destination_marker_contract -v`; `./run_godot.sh self-test`.
