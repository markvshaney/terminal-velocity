...
- [x] Pilot save/resume persistence affordance
  ...
  - Implementation: in-flight `F6` now saves the currently loaded pilot through the same `user://pilots/*.tvpilot.json` path used by New/Open Pilot; the HUD and F10 help expose `F6 save`; the title-screen Open Pilot list shows pilot/ship, saved system, credits, Strict Play status, and most recent saved status before resume; outfitter and shipyard purchase outcomes feed the same recent-message history as mission/trade actions; `./run_godot.sh tv-pilot-save-resume-log` / `RunGodot.ps1 -PilotSaveResumeLog` creates/overwrites a deterministic non-strict test pilot, accepts a courier mission, buys a ship plus outfit/weapon inventory, saves, mutates state including inventory/ship/cargo-space, recent status feedback, and the Strict Play flag, reopens the pilot, and checks round-trip system/fuel/credits/active-mission/Strict-Play/outfit/weapon/ship/cargo-space/status fields.
