...
- [x] EV Classic map multi-stop Shift-click route planning
  ...
  - Implementation: Godot map route selection uses `selected_route` with tail-linked append, invalid-click/current-system route preservation, and full green route drawing; symbolic scenarios `shift_click_multi_stop_route_queue` and `route_queue_invalid_stop_guardrail` verify Levo → Sol → Sirius queue construction, first-leg consumption, and route preservation after duplicate/current-system or unlinked-stop attempts.
  - Verification: `python3 -m unittest native_ev.tests.test_scenario_eval.ScenarioEvalHarnessTests.test_shift_click_multi_stop_route_queue_draws_green_path_and_consumes_first_leg native_ev.tests.test_scenario_eval.ScenarioEvalHarnessTests.test_route_queue_invalid_stop_guardrail_preserves_valid_route -v`; `./run_godot.sh tv-map-route-log`; `./run_godot.sh tv-route-invalid-log`; full native tests and `./run_godot.sh self-test` pass with `gameplayScenarios=16`.
  - Next action: capture an original-runtime step-by-step Basilisk pass when input/capture is reliable enough to verify exact edge cases such as route clearing and path truncation.
