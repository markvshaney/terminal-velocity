...
- [x] Event log for target acquisition, weapon firing, projectile spawn, hit, and explosion.
  - Status: `terminal-velocity-observed`; `./run_godot.sh tv-combat-log` prints target selection, player/NPC projectile spawning, hit/damage fields, a deterministic final-hit destruction setup, and `explosionTriggered=true` with `explosionSourceLabel=terminal-velocity-explosion-visual-scaffold`. `./run_godot.sh tv-legal-consequence-log` now also drives an actual final-hit projectile into a hostile legal patrol scaffold and verifies `combatExecuted=true`, `targetDestroyed=true`, `explosionTriggered=true`, and legal/reputation deltas. `./run_godot.sh tv-player-disabled-log` isolates disabled-player guardrails and F8 recovery under `sourceLabel=terminal-velocity-player-disabled-scaffold` / `oracleStatus=classic_runtime_player_death_pending_strict_play_safe_trace`. Exact Classic combat timing, reward, legal consequence, death/reload, and explosion-frame fidelity remain pending original-runtime/resource comparison.
