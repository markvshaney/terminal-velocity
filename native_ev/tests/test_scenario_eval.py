...
class ScenarioEvalHarnessTests(unittest.TestCase):
    ...
    def test_strategy_skill_rotation_loop_records_distinct_playstyle_checkpoints(self):
        ...
        self.assertEqual(result['checks']['surfaced_strategy_cargo_context'], 'passed')
        ...
        self.assertEqual(checkpoints[0]['heldTradeCargoTons'], 10)
        self.assertEqual(checkpoints[0]['reservedCargoTons'], 0)
        self.assertEqual(checkpoints[1]['heldTradeCargoTons'], 10)
        self.assertEqual(checkpoints[1]['heldTradeCargo']['food'], 10)
        self.assertEqual(checkpoints[1]['reservedCargoTons'], 3)
        self.assertEqual(checkpoints[2]['activeJobs'], ['intro_courier_earth_hera'])
        self.assertEqual(checkpoints[2]['reservedCargoTons'], 3)
        self.assertEqual(checkpoints[2]['heldTradeCargoTons'], 10)
    ...
    def test_upgrade_readiness_strategy_loop_records_service_outfit_weapon_and_ship_checkpoints(self):
        ...
        self.assertEqual(result['checks']['surfaced_upgrade_inventory_context'], 'passed')
        ...
        self.assertEqual(checkpoints[0]['playerShipId'], 'shuttlecraft')
        self.assertEqual(checkpoints[0]['ownedOutfits'], {})
        self.assertEqual(checkpoints[0]['ownedWeapons'], {})
        self.assertEqual(checkpoints[1]['ownedOutfits'].get('cargo_pod'), 1)
        self.assertEqual(checkpoints[1]['cargoCapacity'], 30)
        self.assertEqual(checkpoints[2]['ownedWeapons'].get('laser_cannon'), 1)
        self.assertEqual(checkpoints[-1]['playerShipId'], 'light_freighter')
        self.assertEqual(checkpoints[-1]['cargoCapacity'], 150)
        self.assertEqual(checkpoints[-1]['maxFuel'], 300)
        self.assertEqual(checkpoints[-1]['maxHull'], 300)
        self.assertEqual(result['state']['currentSystem'], 'Sol')
        self.assertEqual(result['state']['landedBody'], 'Earth')
    ...
    def test_mission_trade_destination_sale_loop_sells_cargo_after_delivery(self):
        ...
        self.assertEqual(result['checks']['surfaced_delivery_cargo_release_context'], 'passed')
        ...
        completion = [event for event in result['trace'] if event['type'] == 'complete_cargo_job'][-1]
        self.assertEqual(completion['cargoUsedBefore'], 13)
        self.assertEqual(completion['releasedReservedCargoTons'], 3)
        self.assertEqual(completion['cargoUsed'], 10)
        self.assertEqual(completion['heldTradeCargoTonsAfter'], COMMODITY_LOT_SIZE)
        self.assertEqual(completion['heldTradeCargoAfter']['food'], COMMODITY_LOT_SIZE)
        trade_events = [event for event in result['trace'] if event['type'] in {'buy_commodity_lot', 'sell_commodity_lot'}]
    ...
    def test_mission_trade_return_margin_guardrail_skips_bad_return_cargo_and_finishes_chain(self):
        ...
        self.assertEqual(rejected['lotTons'], COMMODITY_LOT_SIZE)
        self.assertEqual(rejected['projectedLotProfit'], -100)
        self.assertEqual(rejected['cargoUsed'], 0)
        self.assertEqual(rejected['cargoCapacity'], 20)
        self.assertEqual(rejected['freeCargoTons'], 20)
        self.assertEqual(rejected['reservedCargoTons'], 0)
        self.assertEqual(rejected['heldTradeCargoTons'], 0)
        self.assertTrue(rejected['enoughFreeCargoForLot'])
        self.assertEqual(rejected['recoveryHint'], 'skip non-positive return cargo and finish the active contract chain')
    ...
    def test_mission_trade_refuel_delivery_loop_blocks_low_fuel_then_delivers_with_trade_cargo(self):
        ...
        self.assertEqual(result['checks']['surfaced_route_fuel_cargo_context'], 'passed')
        ...
        route_fuel = [event for event in result['trace'] if event.get('type') == 'route_fuel_status'][-1]
        self.assertEqual(route_fuel['activeMissionIds'], ['intro_courier_earth_hera'])
        self.assertEqual(route_fuel['reservedCargoTons'], 3)
        self.assertEqual(route_fuel['heldTradeCargoTons'], COMMODITY_LOT_SIZE)
        self.assertEqual(route_fuel['heldTradeCargo']['food'], COMMODITY_LOT_SIZE)
        self.assertEqual(route_fuel['sourceLabel'], 'terminal-velocity-mission-trade-route-fuel-cargo-scaffold')
        self.assertEqual(route_fuel['oracleStatus'], 'mission_trade_route_fuel_cargo_pending_classic_runtime_trace')
    ...
    def test_static_topology_source_readiness_scout_records_lane_a_promotion_boundary(self):
        ...
        byte_gap = readiness['resourceBibleSystSequentialProjectionByteSizePaddingGap']
        self.assertEqual(byte_gap['sourceLabel'], 'resource-bible-backed-syst-byte-size-padding-gap-negative-oracle')
        self.assertEqual(byte_gap['recordBytes'], 88)
        ...
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketCustodyAuditSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-custody-audit')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketCustodyAuditOracleStatus'], 'syst_template_offset_packet_custody_blocked_pending_replayable_audit_trail')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketCustodyAuditInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketReentryGuardrailSummary', readiness['systTemplateOffsetEvidencePacketCustodyAuditInputs'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketCustodyAuditSchemaVersion'], 1)
        self.assertIn('verifier-replay-missing-or-failed', readiness['systTemplateOffsetEvidencePacketCustodyAuditFailureClassIds'])
        self.assertIn('recovered-packet-contract-matrix-triage-rerun', readiness['systTemplateOffsetEvidencePacketCustodyAuditReentryStepIds'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketCustodyAuditCheckpointCount'], 4)
        self.assertIn('packet-origin-and-hash-readback', readiness['systTemplateOffsetEvidencePacketCustodyAuditCheckpointIds'])
        self.assertIn('do not replay a recovered packet when its artifact hash or origin trail is missing', readiness['systTemplateOffsetEvidencePacketCustodyAuditBlockedShortcuts'])
        self.assertIn('python3 -m json.tool native_ev/data/sourced_ev_systems.json', readiness['systTemplateOffsetEvidencePacketCustodyAuditRequiredVerifiers'])
        self.assertIn('treating custody/audit cleanliness as proof of Classic byte offsets without narrow accepted field claims', readiness['systTemplateOffsetEvidencePacketCustodyAuditBlockedClaims'])
        self.assertIn('custody audit is a replay/audit trail, not Classic byte-offset evidence', readiness['systTemplateOffsetEvidencePacketCustodyAuditPromotionBlockers'])
        self.assertIn('not-promoted', readiness['systTemplateOffsetEvidencePacketCustodyAuditPromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['systTemplateOffsetEvidencePacketCustodyAuditSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-custody-audit')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketPromotionQuarantineSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-promotion-quarantine')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketPromotionQuarantineOracleStatus'], 'syst_template_offset_packet_promotion_quarantine_blocked_pending_accepted_packet_diff')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketPromotionQuarantineInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketCustodyAuditSummary', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineInputs'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketPromotionQuarantineSchemaVersion'], 1)
        self.assertIn('packet-origin-and-hash-readback', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineCustodyCheckpointIds'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketPromotionQuarantineControlCount'], 4)
        self.assertIn('covered-field-only-diff-boundary', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineControlIds'])
        self.assertIn('do not treat a template/source packet as closing record-name joins, display scaling, route-label, or broad universe replacement gates', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineBlockedShortcuts'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineRequiredVerifiers'])
        self.assertIn('using a narrow accepted field packet to promote unrelated syst offsets, display calibration, record-name joins, route labels, non-topology behavior, or broad runtime universe replacement', readiness['systTemplateOffsetEvidencePacketPromotionQuarantineBlockedClaims'])
        self.assertIn('promotion quarantine is a diff boundary, not Classic byte-offset evidence', readiness['systTemplateOffsetEvidencePacketPromotionQuarantinePromotionBlockers'])
        self.assertIn('not-promoted', readiness['systTemplateOffsetEvidencePacketPromotionQuarantinePromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['systTemplateOffsetEvidencePacketPromotionQuarantineSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-promotion-quarantine')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackReadinessSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-readiness')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackReadinessOracleStatus'], 'syst_template_offset_packet_rollback_readiness_blocked_pending_failed_or_conflicting_packet_diff')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackReadinessInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketPromotionQuarantineSummary', readiness['systTemplateOffsetEvidencePacketRollbackReadinessInputs'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackReadinessSchemaVersion'], 1)
        self.assertIn('covered-field-only-diff-boundary', readiness['systTemplateOffsetEvidencePacketRollbackReadinessQuarantineControlIds'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackReadinessControlCount'], 4)
        self.assertIn('failed-verifier-revert-path', readiness['systTemplateOffsetEvidencePacketRollbackReadinessControlIds'])
        self.assertIn('do not patch around a failed verifier by broadening unrelated blockers or weakening model/scenario assertions', readiness['systTemplateOffsetEvidencePacketRollbackReadinessBlockedShortcuts'])
        self.assertIn('python3 tools/extract_ev_system_semantics.py', readiness['systTemplateOffsetEvidencePacketRollbackReadinessRequiredVerifiers'])
        self.assertIn('treating rollback as proof that a rejected or conflicting Classic packet was partially accepted', readiness['systTemplateOffsetEvidencePacketRollbackReadinessBlockedClaims'])
        self.assertIn('rollback readiness is a recovery guardrail, not Classic byte-offset evidence', readiness['systTemplateOffsetEvidencePacketRollbackReadinessPromotionBlockers'])
        self.assertIn('not-promoted', readiness['systTemplateOffsetEvidencePacketRollbackReadinessPromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['systTemplateOffsetEvidencePacketRollbackReadinessSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-readiness')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackRehearsalSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-rehearsal')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackRehearsalOracleStatus'], 'syst_template_offset_packet_rollback_rehearsal_blocked_pending_actual_reverted_packet_diff')
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackRehearsalInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketRollbackReadinessSummary', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalInputs'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackRehearsalSchemaVersion'], 1)
        self.assertIn('failed-verifier-revert-path', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalControlIds'])
        self.assertEqual(readiness['systTemplateOffsetEvidencePacketRollbackRehearsalStepCount'], 4)
        self.assertIn('post-revert-verifier-replay-capture', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalStepIds'])
        self.assertIn('do not mark a rollback rehearsal ready from stale pre-revert verifier output', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalBlockedShortcuts'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalRequiredVerifiers'])
        self.assertIn('treating a dry-run rollback rehearsal as evidence that a Classic offset packet was accepted', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalBlockedClaims'])
        self.assertIn('rollback rehearsal is an operational safety checklist, not Classic byte-offset evidence', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalPromotionBlockers'])
        self.assertIn('not-promoted', readiness['systTemplateOffsetEvidencePacketRollbackRehearsalPromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['systTemplateOffsetEvidencePacketRollbackRehearsalSourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-rehearsal')
        ...
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixSourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-validation-matrix')
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixOracleStatus'], 'non_topology_syst_capture_validation_blocked_pending_complete_control_packets')
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixInputSummaryCount'], 4)
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixFirstProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixFirstWordIndices'], [22, 23, 24, 25, 26])
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixPacketCount'], 2)
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', readiness['nonTopologySystRuntimeCaptureValidationMatrixPacketIds'])
        self.assertEqual(readiness['nonTopologySystRuntimeCaptureValidationMatrixRowCount'], 4)
        self.assertIn('positive-negative-control-pair-completeness', readiness['nonTopologySystRuntimeCaptureValidationMatrixCheckIds'])
        self.assertIn('runtime-observation-used-as-offset-oracle', readiness['nonTopologySystRuntimeCaptureValidationMatrixFailureClassIds'])
        self.assertIn('do not assign Govt/Message/Asteroids/Interference/VisBit offsets from runtime observation alone', readiness['nonTopologySystRuntimeCaptureValidationMatrixBlockedShortcuts'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', readiness['nonTopologySystRuntimeCaptureValidationMatrixRequiredVerifiers'])
        ...
        self.assertEqual(readiness['runtimeUniverseReplacementGateSourceLabel'], 'decoded-resource-backed-runtime-universe-replacement-gate')
        self.assertEqual(readiness['runtimeUniverseReplacementGateOracleStatus'], 'runtime_universe_replacement_blocked_pending_name_topology_display_oracles')
        self.assertEqual(readiness['runtimeUniverseReplacementGateDecodedSystRecordCount'], 67)
        self.assertIn('keep existing runtime universe subset', readiness['runtimeUniverseReplacementGateCurrentRuntimeSubsetPolicy'])
        self.assertEqual(readiness['runtimeUniverseReplacementGateExactMappedResourceIds'], [128])
        self.assertEqual(readiness['runtimeUniverseReplacementGateUnjoinedRecordCount'], 66)
        self.assertEqual(readiness['runtimeUniverseReplacementGateUnjoinedResourceIdRange'], [129, 194])
        self.assertIn('coordinate_display_units_map_scaling_projection', readiness['runtimeUniverseReplacementGateBlockingGateNames'])
        ...
        self.assertEqual(readiness['runtimeRouteLabelProbeExecutionGateCapturePacketSchemaVersion'], 1)
        self.assertEqual(readiness['runtimeRouteLabelProbeExecutionGateRequiredCapturePacketCount'], 3)
        self.assertEqual(readiness['runtimeRouteLabelProbeExecutionGateCapturePacketTargetResourceIds'], [129, 130, 131])
        self.assertEqual(readiness['runtimeRouteLabelProbeExecutionGateCapturePacketSlotNames'], ['Con2', 'Con3', 'Con4'])
        ...
        self.assertEqual(readiness['coordinateDisplayCalibrationGateSourceLabel'], 'decoded-resource-backed-coordinate-display-calibration-gate')
        self.assertEqual(readiness['coordinateDisplayCalibrationGateOracleStatus'], 'coordinate_display_calibration_blocked_pending_classic_map_evidence')
        self.assertEqual(readiness['coordinateDisplayCalibrationGateCandidateInputSummaryCount'], 8)
        self.assertIn('coordinateDisplayScaleInterpretationSummary', readiness['coordinateDisplayCalibrationGateCandidateInputSummaries'])
        self.assertIn('coordinateDisplayQuantizationSummary', readiness['coordinateDisplayCalibrationGateCandidateInputSummaries'])
        self.assertIn('display pixel scale or accepted projection surrogate', readiness['coordinateDisplayCalibrationGateRequiredClaims'])
        self.assertIn('blocked', readiness['coordinateDisplayCalibrationGateReadinessStatus'])
        ...
        self.assertEqual(readiness['coordinateDisplayUnitMapScalingReadinessSourceLabel'], 'decoded-resource-backed-coordinate-display-unit-map-scaling-readiness')
        self.assertEqual(readiness['coordinateDisplayUnitMapScalingReadinessOracleStatus'], 'coordinate_display_units_map_scaling_bounded_static_readiness_promoted')
        self.assertEqual(readiness['coordinateDisplayUnitMapScalingReadinessCandidateInputSummaryCount'], 10)
        self.assertIn('coordinateDisplayRuntimeCaptureGateSummary', readiness['coordinateDisplayUnitMapScalingReadinessCandidateInputSummaries'])
        self.assertIn('complete decoded xPos/yPos coordinate fields for all 67 syst-like records', readiness['coordinateDisplayUnitMapScalingReadinessPromotedClaims'])
        self.assertIn('Classic map pixel scale, projection, origin, and centering are still unpromoted', readiness['coordinateDisplayUnitMapScalingReadinessBlockedClaims'])
        self.assertEqual(readiness['coordinateDisplayUnitMapScalingReadinessRequiredRuntimeCaptureTargetResourceIds'], [129, 130, 131])
        self.assertEqual(readiness['coordinateDisplayUnitMapScalingReadinessStatus'], 'ready-for-runtime-calibration-capture; display units not promoted')
        self.assertIn('not-promoted', readiness['coordinateDisplayUnitMapScalingReadinessDisplayUnitPromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['coordinateDisplayUnitMapScalingReadinessSourceLabel'], 'decoded-resource-backed-coordinate-display-unit-map-scaling-readiness')
        ...
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationSourceLabel'], 'original-runtime-coordinate-display-capture-validation-matrix')
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationOracleStatus'], 'coordinate_display_capture_validation_blocked_pending_real_map_packets')
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationInputSummaryCount'], 4)
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationTargetResourceIds'], [129, 130, 131])
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationConSlots'], ['Con2', 'Con3', 'Con4'])
        self.assertIn('map-pixel-observation-completeness', readiness['coordinateDisplayRuntimeCaptureValidationCheckIds'])
        self.assertIn('promotion-boundary-contamination', readiness['coordinateDisplayRuntimeCaptureValidationFailureClassIds'])
        self.assertEqual(readiness['coordinateDisplayRuntimeCaptureValidationMinimumAcceptedPacketCount'], 2)
        self.assertIn('not-promoted', readiness['coordinateDisplayRuntimeCaptureValidationPromotionStatus'])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['coordinateDisplayRuntimeCaptureValidationSourceLabel'], 'original-runtime-coordinate-display-capture-validation-matrix')
        ...
        self.assertEqual(readiness['systemNameLandingProximitySourceLabel'], 'decoded-resource-backed-system-name-landing-proximity-scout')
        self.assertEqual(readiness['systemNameLandingProximityOracleStatus'], 'exact_record_name_runtime_topology_mapping_pending')
        self.assertIn('system-name text seed to nearest landing-name text seed byte-proximity candidates', readiness['systemNameLandingProximityCandidateFamilies'])
        self.assertEqual(readiness['systemNameLandingProximityCloseCandidates'], ['Centauri', 'Sirius', 'Tau Ceti', 'Alkaid', 'Zaxted', 'Clotho'])
        self.assertEqual(readiness['systemNameLandingProximityDistantCandidates'], ['Sol', 'Enyo', 'Antares'])
        self.assertEqual(readiness['systemNameLandingProximityExactMappedLandingCandidates'], [{'systemName': 'Levo', 'landingNameSeedByteOffsets': [23867]}])
        self.assertEqual(result['state']['sourceReadiness']['staticTopology']['systemNameLandingProximitySourceLabel'], 'decoded-resource-backed-system-name-landing-proximity-scout')
    ...
    def test_system_service_provisioning_scout_records_service_matrix_boundaries(self):
        ...
        self.assertEqual(result['checks']['recorded_service_store_evidence_packet_contract'], 'passed')
        ...
        self.assertEqual(result['checks']['recorded_service_store_evidence_packet_recovery_plan'], 'passed')
        self.assertEqual(result['checks']['recorded_service_store_evidence_packet_custody_guardrail'], 'passed')
        self.assertEqual(result['checks']['recorded_service_store_evidence_packet_promotion_quarantine'], 'passed')
