...
class NativeEvModelTests(unittest.TestCase):
    ...
    def test_godot_help_exposes_gameplay_curriculum_hints_contract(self):
        ...
        for expected_line in [
            ...
            'Alignment branch delivery recovery: after accepting a chapter branch, keep the chosen destination/cargo goal stable, deliver it before taking the incompatible branch, then verify Mission Log/Player Info flags and cargo release. TV mission scaffold; Classic branch completion UI pending.',
            ...
            'Upgrade persistence recovery: after buying outfits, weapons, or a new hull, press F6, reopen the pilot from the title screen, and verify Player Info still shows the purchased ship/outfits/weapons before taking riskier jobs. TV save/resume scaffold; exact Classic pilot-file UX pending.',
            'Mission persistence recovery: after accepting a job and saving, reopen the pilot from the title screen and verify Mission Log/Player Info still show the active cargo, route, strict-play, status-message, and upgrade checkpoints before changing cargo plans. TV save/resume scaffold; exact Classic pilot-file UX pending.',
            'Outfitter/shipyard launch recovery: if B is blocked because you are in space, land at the intended service port first, keep the selected purchase goal, then retry only after checking credits and local stock. TV service-purchase guardrail scaffold; Classic purchase UI pending.',
            ...
            'Combat reward/salvage recovery: after a disable creates both reward history and full-hold salvage, verify reward history stays visible, free hold space by selling or delivering cargo, then recover the marker before leaving. TV scaffold; Classic bounty/loot coupling pending.',
            'Commodity partial-hold recovery: if Buy reports the hold needs a full 10-ton lot, sell or deliver cargo until 10 tons are free before retrying the commodity purchase. TV scaffold; exact Classic partial-hold UI pending.',
            'Commodity exact-credit recovery: if the price exactly matches your remaining credits, Buy should succeed and leave 0 credits; verify cargo and budget before buying another lot. TV commodity-buy scaffold; exact Classic edge-case UI pending.',
            'Cross-market exact-credit profit recovery: if a linked-market lot spends your last credits, keep the selected return route, avoid another buy until you sell the held lot, then verify credits recover at the destination market. TV cross-market scaffold; Classic spread/edge-case UI pending.',
        ...
        for flag in [
            ...
            'hasCombatRewardSalvageRecoveryHelp',
            'hasSalvageHelp',
            ...
            'hasAlignmentBranchDeliveryRecoveryHelp',
            ...
            'hasCommodityUnavailableRecoveryHelp',
            'hasCommodityPartialHoldRecoveryHelp',
            'hasCommodityExactCreditRecoveryHelp',
            'hasCrossMarketExactCreditProfitRecoveryHelp',
            'hasTradeRouteHelp',
            'hasMapServiceHelp',
            ...
            'hasOutfitterShipyardLaunchRecoveryHelp',
            'hasShipyardCargoTransferHelp',
            'hasShipLadderHelp',
            'hasPilotPersistenceHelp',
            'hasUpgradePersistenceRecoveryHelp',
            'hasMissionPersistenceRecoveryHelp',
            'hasPlayerInfoHelp',
    ...
    def test_godot_navigation_blocked_reasons_are_player_guidance_contract(self):
        ...
        for symbol in [
            ...
            'Route fuel: %d hop(s), cost %d, fuel %d/%d, reserve %+d',
            'func _route_fuel_margin',
            'func _nearest_refuel_body_name',
            'warning += " at "',
            'preJumpFuelWarning=%s',
            'routeFuelMargin=%d',
            'tooCloseGuidance=%s',
            'Route selected: %s — fuel cost %d, fuel %d/%d, reserve %+d — press J to jump',
    ...
    def test_godot_legal_status_surface_is_scaffold_contract(self):
        ...
        for symbol in [
            ...
            'serviceLegalMinScore=%d',
            'legalScoreJustBelowServiceMin=%d',
            'serviceBlockedJustBelowMin=%s',
            'legalScoreAtServiceMin=%d',
            'outfitterRecoveredAtMin=%s',
            'shipyardRecoveredAtMin=%s',
            'serviceClemencyPaid=%s',
            'legalAfterServiceClemency=%d',
            'outfitterRecoveredAfterClemency=%s',
            'shipyardRecoveredAfterClemency=%s',
            ...
            'legalScoreJustBelowTolerance=%d',
            'patrolHostileJustBelowTolerance=%s',
            'legalScoreAtTolerance=%d',
            'patrolRecoveredAtTolerance=%s',
            'combatExecuted=true',
            'projectileSpawned=%s',
            'targetDestroyed=%s',
            'explosionTriggered=%s',
            ...
            'blockedReasons=%s',
            'missionLegalMinScore=%d',
            'legalScoreJustBelowMissionMin=%d',
            'missionBlockedJustBelowMin=%s',
            'legalScoreAtMissionMin=%d',
            'missionRecoveredAtMin=%s',
            'missionLegalMaxScore=%d',
            'legalScoreJustAboveMissionMax=%d',
            'missionBlockedJustAboveMax=%s',
            'legalScoreAtMissionMax=%d',
            'missionRecoveredAtMax=%s',
            'legalMaxBlockedReason=\\"%s\\"',
            'above allowed %d; TV scaffold, exact Classic mission gates unconfirmed',
            ...
            'dockMinLegalScore=%d',
            'legalScoreJustBelowDockMin=%d',
            'dockingDeniedJustBelowMin=%s',
            'legalScoreAtDockMin=%d',
            'dockingRecoveredAtMin=%s',
    ...
    def test_godot_map_route_autoresearch_log_contract(self):
        ...
        for symbol in [
            ...
            'routeHops=%d route=%s',
            'preJumpFuelWarning=%s',
            'Route appended: %s — fuel cost %d, fuel %d/%d, reserve %+d',
    ...
    def test_godot_mission_destination_route_hint_log_contract(self):
        ...
        for symbol in [
            ...
            'Route fuel: %d hop(s), cost %d, fuel %d/%d, reserve %+d%s',
            'warning += " at "',
    ...
    def test_godot_shipyard_cargo_guardrail_log_contract(self):
        root = Path(__file__).resolve().parents[2]
        main_script = _godot_contract_text(root)
        run_script = (root / 'godot_ev' / 'windows' / 'RunGodot.ps1').read_text()
        wrapper = (root / 'run_godot.sh').read_text()
        for symbol in [
            '--tv-shipyard-cargo-guardrail-log',
            'func _run_shipyard_cargo_guardrail_log() -> void:',
            'TV_SHIPYARD_CARGO_GUARDRAIL_EVENT',
            'overfullShipyardBlocked=%s',
            'overfullCargoPreserved=%s',
            'recoveryBoughtSmallerShip=%s',
            'blockedShip=shuttlecraft',
            'sourceLabel=terminal-velocity-shipyard-cargo-guardrail-scaffold',
            'oracleStatus=shipyard_cargo_transfer_pending_ev_classic_runtime_trace',
        ]:
            self.assertIn(symbol, main_script)
        self.assertIn('[switch]$ShipyardCargoGuardrailLog', run_script)
        self.assertIn('--headless --path $Project -- --tv-shipyard-cargo-guardrail-log', run_script)
        self.assertIn('tv-shipyard-cargo-guardrail-log', wrapper)
    ...
    def test_godot_pilot_save_resume_log_contract(self):
        ...
        for symbol in [
            ...
            'resumedUpgradePlayerInfoVisible=true',
            'resumedPlayerInfoLines=%s',
    ...
    def test_sourced_ev_systems_manifest_promotes_static_system_ids_and_name_seeds(self):
        ...
        self.assertEqual(topology_constants['sourceLabel'], 'resource-bible-backed-topology-constants')
        self.assertEqual(topology_constants['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(topology_constants['maxStellarObjects'], 1500)
        self.assertEqual(topology_constants['maxSystems'], 1000)
        self.assertEqual(topology_constants['jumpDistancePixels'], 1000)
        self.assertIn('Resource Bible game constants for topology capacity and hyperspace range', topology_constants['candidateFamilies'])
        self.assertIn('not a decoded syst coordinate unit interpretation', ' '.join(topology_constants['promotionBlockers']))
        self.assertIn('does not claim that decoded xPos/yPos raw words are already expressed in map pixels', topology_constants['sourceNote'])
        coordinate_map_source = data['coordinateMapSourceReadinessSummary']
        self.assertEqual(coordinate_map_source['sourceLabel'], 'resource-bible-backed-coordinate-map-source-readiness')
        self.assertEqual(coordinate_map_source['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(coordinate_map_source['sourceReferences']['mapPlacementFields'], 'docs/references/ev-family/ev-classic-resource-bible.txt lines 924-937')
        self.assertEqual(coordinate_map_source['recordCount'], 67)
        self.assertEqual(coordinate_map_source['coordinateFieldCompleteRecordCount'], 67)
        self.assertEqual(coordinate_map_source['linkSlotCompleteRecordCount'], 67)
        self.assertEqual(coordinate_map_source['resourceBibleMapPlacementClaim'], 'The sÿst resource xPos/yPos fields are the system X and Y positions on the map.')
        self.assertEqual(coordinate_map_source['resourceBibleRangeConstantBoundary'], 'JumpDistance=1000 pixels is preserved as a game/topology range constant, not as decoded xPos/yPos map-pixel proof.')
        ...
        field_order_conflict = data['systFieldOrderConflictSummary']
        self.assertEqual(field_order_conflict['sourceLabel'], 'decoded-resource-backed-syst-field-order-conflict-scout')
        self.assertEqual(field_order_conflict['oracleStatus'], 'syst_field_order_mapping_pending_complete_oracle')
        conflict_scouts = field_order_conflict['decodedWordGroupScouts']
        ...
        byte_size_gap = field_count_budget['byteSizePaddingGapSummary']
        self.assertEqual(byte_size_gap['sourceLabel'], 'resource-bible-backed-syst-byte-size-padding-gap-negative-oracle')
        self.assertEqual(byte_size_gap['recordBytes'], 88)
        ...
        template_offset_gap = data['systTemplateOffsetOracleGapSummary']
        self.assertEqual(template_offset_gap['sourceLabel'], 'repo-reference-backed-syst-resedit-template-source-availability-gap')
        self.assertEqual(template_offset_gap['oracleStatus'], 'syst_template_offset_mapping_blocked_pending_exact_tmpl_or_resedit_source')
        self.assertEqual(template_offset_gap['evidenceInputSummaryCount'], 4)
        self.assertIn('resourceBibleSystSequentialFieldProjectionSummary', template_offset_gap['evidenceInputSummaries'])
        self.assertIn('systWordDomainCoverageSummary', template_offset_gap['evidenceInputSummaries'])
        self.assertIn('no exact syst TMPL/resource-template artifact is recorded in the local source manifest', template_offset_gap['missingEvidence'])
        self.assertIn('template/source absence means Resource Bible prose cannot be converted into byte offsets by assertion', template_offset_gap['promotionBlockers'])
        self.assertIn('Classic-specific syst TMPL/ResEdit/source offset oracle is absent', template_offset_gap['topLevelGateStatus'])
        self.assertIn('not-promoted', template_offset_gap['promotionStatus'])
        template_search_priority = data['systTemplateOffsetSourceSearchPrioritySummary']
        self.assertEqual(template_search_priority['sourceLabel'], 'repo-reference-backed-syst-template-offset-source-search-priority')
        self.assertEqual(template_search_priority['oracleStatus'], 'syst_template_offset_search_blocked_pending_classic_specific_evidence')
        self.assertEqual(template_search_priority['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetOracleGapSummary', template_search_priority['evidenceInputSummaries'])
        self.assertEqual(template_search_priority['firstPriorityTargetId'], 'classic-syst-tmpl-or-resedit-template')
        self.assertIn('classic-source-syst-struct', template_search_priority['priorityTargetIds'])
        self.assertIn('validated-runtime-offset-surrogate', template_search_priority['priorityTargetIds'])
        self.assertIn('search priority is a worklist, not Classic byte-offset evidence', template_search_priority['promotionBlockers'])
        self.assertIn('not-promoted', template_search_priority['promotionStatus'])
        self.assertIn('Classic-specific syst TMPL/template or ResEdit field map', template_search_priority['nextEvidenceFamilies'][0])
        template_packet_contract = data['systTemplateOffsetEvidencePacketContractSummary']
        self.assertEqual(template_packet_contract['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-contract')
        self.assertEqual(template_packet_contract['oracleStatus'], 'syst_template_offset_evidence_packets_blocked_pending_validated_classic_specific_packet')
        self.assertEqual(template_packet_contract['evidenceInputSummaryCount'], 5)
        self.assertIn('systTemplateOffsetSourceSearchPrioritySummary', template_packet_contract['evidenceInputSummaries'])
        self.assertEqual(template_packet_contract['firstAcceptedEvidenceClassId'], 'classic-syst-tmpl-or-resedit-template')
        self.assertIn('classic-source-syst-struct', template_packet_contract['acceptedEvidenceClassIds'])
        self.assertIn('validated-runtime-offset-surrogate', template_packet_contract['acceptedEvidenceClassIds'])
        self.assertEqual(template_packet_contract['requiredPacketSchemaVersion'], 1)
        self.assertIn('packet must name Classic-specific provenance before any byte-offset or field-width claim is considered', template_packet_contract['validationRules'])
        self.assertIn('evidence packet contract is an acceptance schema, not Classic byte-offset evidence', template_packet_contract['promotionBlockers'])
        self.assertIn('not-promoted', template_packet_contract['promotionStatus'])
        packet_validation_matrix = data['systTemplateOffsetEvidencePacketValidationMatrixSummary']
        self.assertEqual(packet_validation_matrix['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-validation-matrix')
        self.assertEqual(packet_validation_matrix['oracleStatus'], 'syst_template_offset_packet_validation_blocked_pending_real_packet')
        self.assertEqual(packet_validation_matrix['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketContractSummary', packet_validation_matrix['evidenceInputSummaries'])
        self.assertEqual(packet_validation_matrix['contractSchemaVersion'], 1)
        self.assertIn('classic-source-syst-struct', packet_validation_matrix['contractAcceptedEvidenceClassIds'])
        self.assertEqual(packet_validation_matrix['validationCaseCount'], 4)
        self.assertEqual(packet_validation_matrix['firstValidationCaseId'], 'accept-classic-template-field-offset-packet')
        self.assertIn('reject-ev-family-template-transfer-packet', packet_validation_matrix['rejectionCaseIds'])
        self.assertIn('EV-family/adaptation-only packets must be rejected as Classic syst offset evidence', packet_validation_matrix['requiredVerifierOutcomes'])
        self.assertIn('validation matrix is a verifier fixture, not a validated evidence packet', packet_validation_matrix['promotionBlockers'])
        self.assertIn('not-promoted', packet_validation_matrix['promotionStatus'])
        packet_replay_readiness = data['systTemplateOffsetEvidencePacketReplayReadinessSummary']
        self.assertEqual(packet_replay_readiness['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-replay-readiness')
        self.assertEqual(packet_replay_readiness['oracleStatus'], 'syst_template_offset_packet_replay_blocked_pending_real_packet')
        self.assertEqual(packet_replay_readiness['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketValidationMatrixSummary', packet_replay_readiness['evidenceInputSummaries'])
        self.assertEqual(packet_replay_readiness['contractSchemaVersion'], 1)
        self.assertIn('reject-ev-family-template-transfer-packet', packet_replay_readiness['validationCaseIds'])
        self.assertEqual(packet_replay_readiness['replayStepCount'], 4)
        self.assertEqual(packet_replay_readiness['firstReplayStepId'], 'packet-artifact-readback')
        self.assertIn('local-verifier-replay', [step['stepId'] for step in packet_replay_readiness['replaySteps']])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', packet_replay_readiness['requiredVerifierBeforePromotion'])
        self.assertIn('promoting runtime surrogate packets into Classic byte offsets without Classic source/template corroboration', packet_replay_readiness['blockedPromotionClaims'])
        self.assertIn('replay readiness is a handoff checklist, not a validated evidence packet', packet_replay_readiness['promotionBlockers'])
        self.assertIn('not-promoted', packet_replay_readiness['promotionStatus'])
        packet_intake_triage = data['systTemplateOffsetEvidencePacketIntakeTriageSummary']
        self.assertEqual(packet_intake_triage['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-intake-triage')
        self.assertEqual(packet_intake_triage['oracleStatus'], 'syst_template_offset_packet_triage_blocked_pending_real_packet')
        self.assertEqual(packet_intake_triage['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketReplayReadinessSummary', packet_intake_triage['evidenceInputSummaries'])
        self.assertEqual(packet_intake_triage['contractSchemaVersion'], 1)
        self.assertIn('reject-ev-family-template-transfer-packet', packet_intake_triage['validationCaseIds'])
        self.assertIn('local-verifier-replay', packet_intake_triage['replayStepIds'])
        self.assertEqual(packet_intake_triage['triageDecisionCount'], 4)
        self.assertIn('triage-runtime-surrogate-packet', packet_intake_triage['triageDecisionIds'])
        self.assertIn('triage-ev-family-or-adaptation-only-packet', packet_intake_triage['triageDecisionIds'])
        self.assertIn('reject-as-classic-syst-offset-evidence', [decision['triageOutcome'] for decision in packet_intake_triage['triageDecisions']])
        self.assertIn('intake triage is a routing checklist, not a validated evidence packet', packet_intake_triage['promotionBlockers'])
        self.assertIn('not-promoted', packet_intake_triage['promotionStatus'])
        packet_failure_taxonomy = data['systTemplateOffsetEvidencePacketFailureTaxonomySummary']
        self.assertEqual(packet_failure_taxonomy['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-failure-taxonomy')
        self.assertEqual(packet_failure_taxonomy['oracleStatus'], 'syst_template_offset_packet_failure_taxonomy_blocked_pending_real_packet')
        self.assertEqual(packet_failure_taxonomy['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketIntakeTriageSummary', packet_failure_taxonomy['evidenceInputSummaries'])
        self.assertEqual(packet_failure_taxonomy['contractSchemaVersion'], 1)
        self.assertEqual(packet_failure_taxonomy['failureClassCount'], 5)
        self.assertIn('verifier-replay-missing-or-failed', packet_failure_taxonomy['failureClassIds'])
        self.assertIn('triage-ev-family-or-adaptation-only-packet', packet_failure_taxonomy['triageDecisionIds'])
        self.assertIn('packet verifier output cannot be replayed locally or fails current extractor/model/scenario checks', packet_failure_taxonomy['blockedPromotionClaims'])
        self.assertIn('failure taxonomy is a rejection/recovery map, not Classic byte-offset evidence', packet_failure_taxonomy['promotionBlockers'])
        self.assertIn('not-promoted', packet_failure_taxonomy['promotionStatus'])
        packet_recovery_plan = data['systTemplateOffsetEvidencePacketRecoveryPlanSummary']
        self.assertEqual(packet_recovery_plan['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-recovery-plan')
        self.assertEqual(packet_recovery_plan['oracleStatus'], 'syst_template_offset_packet_recovery_plan_blocked_pending_real_packet')
        self.assertEqual(packet_recovery_plan['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketFailureTaxonomySummary', packet_recovery_plan['evidenceInputSummaries'])
        self.assertEqual(packet_recovery_plan['contractSchemaVersion'], 1)
        self.assertEqual(packet_recovery_plan['failureClassIds'], packet_failure_taxonomy['failureClassIds'])
        self.assertEqual(packet_recovery_plan['recoveryActionCount'], 5)
        self.assertIn('recover-missing-replayable-artifact', packet_recovery_plan['recoveryActionIds'])
        self.assertIn('recover-classic-provenance-absent', packet_recovery_plan['recoveryActionIds'])
        self.assertIn('recover-verifier-replay-missing-or-failed', packet_recovery_plan['recoveryActionIds'])
        self.assertIn('recover-scope-overreach-beyond-covered-fields', packet_recovery_plan['recoveryActionIds'])
        self.assertIn('recover-ev-family-or-adaptation-only-transfer', packet_recovery_plan['recoveryActionIds'])
        self.assertIn('successful local replay of extractor, focused model validation, and static_topology_source_readiness_scout with captured actual output', [action['requiredNextEvidence'] for action in packet_recovery_plan['recoveryActions']])
        self.assertIn('treating a recovery plan as evidence that a previously rejected packet has been fixed', packet_recovery_plan['blockedPromotionClaims'])
        self.assertIn('recovery plan is a next-evidence checklist, not Classic byte-offset evidence', packet_recovery_plan['promotionBlockers'])
        self.assertIn('recovered packets must rerun the full contract, matrix, triage, verifier replay, and narrow-scope review path', packet_recovery_plan['promotionBlockers'])
        self.assertIn('recovery actions may update packet status/search priority only; they must not promote offsets or runtime behavior without accepted evidence', packet_recovery_plan['promotionBlockers'])
        self.assertIn('broad runtime universe replacement remains blocked until all name, topology, display, non-topology, and template-offset gates close', packet_recovery_plan['promotionBlockers'])
        self.assertIn('not-promoted', packet_recovery_plan['promotionStatus'])
        packet_reentry_guardrail = data['systTemplateOffsetEvidencePacketReentryGuardrailSummary']
        self.assertEqual(packet_reentry_guardrail['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-reentry-guardrail')
        self.assertEqual(packet_reentry_guardrail['oracleStatus'], 'syst_template_offset_packet_reentry_blocked_pending_recovered_real_packet')
        self.assertEqual(packet_reentry_guardrail['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketRecoveryPlanSummary', packet_reentry_guardrail['evidenceInputSummaries'])
        self.assertEqual(packet_reentry_guardrail['contractSchemaVersion'], 1)
        self.assertEqual(packet_reentry_guardrail['failureClassIds'], packet_recovery_plan['failureClassIds'])
        self.assertEqual(packet_reentry_guardrail['reentryStepCount'], 4)
        self.assertIn('recovered-packet-contract-matrix-triage-rerun', packet_reentry_guardrail['reentryStepIds'])
        self.assertIn('do not resume from the failure point or skip earlier acceptance gates because a packet was revised', packet_reentry_guardrail['blockedShortcuts'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', packet_reentry_guardrail['requiredVerifierBeforePromotion'])
        self.assertIn('treating a recovered packet as accepted before artifact readback, failure-class closure, full replay, and narrow-scope quarantine', packet_reentry_guardrail['blockedPromotionClaims'])
        self.assertIn('reentry guardrail is a replay path, not Classic byte-offset evidence', packet_reentry_guardrail['promotionBlockers'])
        self.assertIn('recovered packets must start again at artifact readback and rerun every acceptance gate', packet_reentry_guardrail['promotionBlockers'])
        self.assertIn('not-promoted', packet_reentry_guardrail['promotionStatus'])
        packet_custody_audit = data['systTemplateOffsetEvidencePacketCustodyAuditSummary']
        self.assertEqual(packet_custody_audit['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-custody-audit')
        self.assertEqual(packet_custody_audit['oracleStatus'], 'syst_template_offset_packet_custody_blocked_pending_replayable_audit_trail')
        self.assertEqual(packet_custody_audit['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketReentryGuardrailSummary', packet_custody_audit['evidenceInputSummaries'])
        self.assertEqual(packet_custody_audit['contractSchemaVersion'], 1)
        self.assertEqual(packet_custody_audit['failureClassIds'], packet_reentry_guardrail['failureClassIds'])
        self.assertEqual(packet_custody_audit['custodyCheckpointCount'], 4)
        self.assertIn('packet-origin-and-hash-readback', packet_custody_audit['custodyCheckpointIds'])
        self.assertIn('python3 -m json.tool native_ev/data/sourced_ev_systems.json', packet_custody_audit['requiredVerifierBeforePromotion'])
        self.assertIn('do not replay a recovered packet when its artifact hash or origin trail is missing', packet_custody_audit['blockedShortcuts'])
        self.assertIn('treating custody/audit cleanliness as proof of Classic byte offsets without narrow accepted field claims', packet_custody_audit['blockedPromotionClaims'])
        self.assertIn('custody audit is a replay/audit trail, not Classic byte-offset evidence', packet_custody_audit['promotionBlockers'])
        self.assertIn('not-promoted', packet_custody_audit['promotionStatus'])
        packet_promotion_quarantine = data['systTemplateOffsetEvidencePacketPromotionQuarantineSummary']
        self.assertEqual(packet_promotion_quarantine['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-promotion-quarantine')
        self.assertEqual(packet_promotion_quarantine['oracleStatus'], 'syst_template_offset_packet_promotion_quarantine_blocked_pending_accepted_packet_diff')
        self.assertEqual(packet_promotion_quarantine['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketCustodyAuditSummary', packet_promotion_quarantine['evidenceInputSummaries'])
        self.assertEqual(packet_promotion_quarantine['contractSchemaVersion'], 1)
        self.assertEqual(packet_promotion_quarantine['custodyCheckpointIds'], packet_custody_audit['custodyCheckpointIds'])
        self.assertEqual(packet_promotion_quarantine['quarantineControlCount'], 4)
        self.assertIn('covered-field-only-diff-boundary', packet_promotion_quarantine['quarantineControlIds'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', packet_promotion_quarantine['requiredVerifierBeforePromotion'])
        self.assertIn('do not treat a template/source packet as closing record-name joins, display scaling, route-label, or broad universe replacement gates', packet_promotion_quarantine['blockedShortcuts'])
        self.assertIn('promotion quarantine is a diff boundary, not Classic byte-offset evidence', packet_promotion_quarantine['promotionBlockers'])
        self.assertIn('not-promoted', packet_promotion_quarantine['promotionStatus'])
        packet_rollback_readiness = data['systTemplateOffsetEvidencePacketRollbackReadinessSummary']
        self.assertEqual(packet_rollback_readiness['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-readiness')
        self.assertEqual(packet_rollback_readiness['oracleStatus'], 'syst_template_offset_packet_rollback_readiness_blocked_pending_failed_or_conflicting_packet_diff')
        self.assertEqual(packet_rollback_readiness['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketPromotionQuarantineSummary', packet_rollback_readiness['evidenceInputSummaries'])
        self.assertEqual(packet_rollback_readiness['contractSchemaVersion'], 1)
        self.assertEqual(packet_rollback_readiness['quarantineControlIds'], packet_promotion_quarantine['quarantineControlIds'])
        self.assertEqual(packet_rollback_readiness['rollbackControlCount'], 4)
        self.assertIn('failed-verifier-revert-path', packet_rollback_readiness['rollbackControlIds'])
        self.assertIn('python3 tools/extract_ev_system_semantics.py', packet_rollback_readiness['requiredVerifierAfterRollback'])
        self.assertIn('do not patch around a failed verifier by broadening unrelated blockers or weakening model/scenario assertions', packet_rollback_readiness['blockedShortcuts'])
        self.assertIn('rollback readiness is a recovery guardrail, not Classic byte-offset evidence', packet_rollback_readiness['promotionBlockers'])
        self.assertIn('not-promoted', packet_rollback_readiness['promotionStatus'])
        packet_rollback_rehearsal = data['systTemplateOffsetEvidencePacketRollbackRehearsalSummary']
        self.assertEqual(packet_rollback_rehearsal['sourceLabel'], 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-rehearsal')
        self.assertEqual(packet_rollback_rehearsal['oracleStatus'], 'syst_template_offset_packet_rollback_rehearsal_blocked_pending_actual_reverted_packet_diff')
        self.assertEqual(packet_rollback_rehearsal['evidenceInputSummaryCount'], 4)
        self.assertIn('systTemplateOffsetEvidencePacketRollbackReadinessSummary', packet_rollback_rehearsal['evidenceInputSummaries'])
        self.assertEqual(packet_rollback_rehearsal['contractSchemaVersion'], 1)
        self.assertEqual(packet_rollback_rehearsal['rollbackControlIds'], packet_rollback_readiness['rollbackControlIds'])
        self.assertEqual(packet_rollback_rehearsal['rehearsalStepCount'], 4)
        self.assertIn('post-revert-verifier-replay-capture', packet_rollback_rehearsal['rehearsalStepIds'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', packet_rollback_rehearsal['requiredVerifierAfterRehearsal'])
        self.assertIn('do not mark a rollback rehearsal ready from stale pre-revert verifier output', packet_rollback_rehearsal['blockedShortcuts'])
        self.assertIn('rollback rehearsal is an operational safety checklist, not Classic byte-offset evidence', packet_rollback_rehearsal['promotionBlockers'])
        self.assertIn('not-promoted', packet_rollback_rehearsal['promotionStatus'])
        ...
        runtime_probe_priority = data['nonTopologySystRuntimeProbePrioritySummary']
        self.assertEqual(runtime_probe_priority['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-probe-priority')
        self.assertEqual(runtime_probe_priority['oracleStatus'], 'non_topology_syst_runtime_probe_blocked_pending_offset_or_runtime_oracle')
        self.assertEqual(runtime_probe_priority['recordCount'], 67)
        self.assertEqual(runtime_probe_priority['evidenceInputSummaryCount'], 4)
        self.assertIn('runtimeUniverseReplacementGateSummary', runtime_probe_priority['evidenceInputSummaries'])
        self.assertEqual(runtime_probe_priority['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_probe_priority['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertEqual(runtime_probe_priority['percentLikeValueWords'], [20, 21, 22, 23])
        self.assertEqual(runtime_probe_priority['zeroOnlyTailWords'], list(range(24, 44)))
        self.assertIn('Govt/Message/Asteroids/Interference/VisBit environment and visibility controls', runtime_probe_priority['blockedResourceBibleFamilies'])
        self.assertIn('priority groups are a capture worklist, not promoted syst field offsets', runtime_probe_priority['promotionBlockers'])
        self.assertIn('runtime captures comparing government/message/hazard/visibility behavior', runtime_probe_priority['nextEvidenceFamilies'][0])
        self.assertIn('not-promoted', runtime_probe_priority['promotionStatus'])
        runtime_capture_gate = data['nonTopologySystRuntimeCaptureGateSummary']
        self.assertEqual(runtime_capture_gate['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-gate')
        self.assertEqual(runtime_capture_gate['oracleStatus'], 'non_topology_syst_capture_blocked_pending_disposable_runtime_probe')
        self.assertEqual(runtime_capture_gate['recordCount'], 67)
        self.assertEqual(runtime_capture_gate['evidenceInputSummaryCount'], 3)
        self.assertIn('nonTopologySystRuntimeProbePrioritySummary', runtime_capture_gate['evidenceInputSummaries'])
        self.assertEqual(runtime_capture_gate['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_capture_gate['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertIn('government ownership or landing/legal-affiliation behavior', runtime_capture_gate['requiredComparisonAxes'])
        self.assertIn('disposableNonStrictPilot', runtime_capture_gate['requiredCaptureFields'])
        self.assertEqual(runtime_capture_gate['capturePacketSchemaVersion'], 1)
        self.assertEqual(runtime_capture_gate['requiredCapturePacketCount'], 2)
        packet_ids = [entry['packetId'] for entry in runtime_capture_gate['capturePacketTemplates']]
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', packet_ids)
        self.assertIn('non-topology-gov-message-hazard-visibility-negative-control', packet_ids)
        self.assertIn('positive and negative controls must be recorded before any field-order promotion claim', runtime_capture_gate['captureValidationRules'])
        self.assertIn('do not run destructive or reputation-changing probes on Strict Play or reusable pilots', runtime_capture_gate['safetyBlockers'])
        self.assertIn('not-promoted', runtime_capture_gate['promotionStatus'])
        runtime_capture_validation_matrix = data['nonTopologySystRuntimeCaptureValidationMatrixSummary']
        self.assertEqual(runtime_capture_validation_matrix['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-validation-matrix')
        self.assertEqual(runtime_capture_validation_matrix['oracleStatus'], 'non_topology_syst_capture_validation_blocked_pending_complete_control_packets')
        self.assertEqual(runtime_capture_validation_matrix['recordCount'], 67)
        self.assertEqual(runtime_capture_validation_matrix['evidenceInputSummaryCount'], 4)
        self.assertIn('nonTopologySystRuntimeCaptureGateSummary', runtime_capture_validation_matrix['evidenceInputSummaries'])
        self.assertEqual(runtime_capture_validation_matrix['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_capture_validation_matrix['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertEqual(runtime_capture_validation_matrix['requiredValidatedCapturePacketCount'], 2)
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', runtime_capture_validation_matrix['requiredCapturePacketIds'])
        self.assertEqual(runtime_capture_validation_matrix['validationRowCount'], 4)
        self.assertIn('positive-negative-control-pair-completeness', runtime_capture_validation_matrix['validationCheckIds'])
        self.assertIn('runtime-observation-used-as-offset-oracle', runtime_capture_validation_matrix['failureClassIds'])
        self.assertIn('do not assign Govt/Message/Asteroids/Interference/VisBit offsets from runtime observation alone', runtime_capture_validation_matrix['blockedShortcuts'])
        self.assertIn('capture validation matrix is an acceptance checklist, not Classic byte-offset evidence', runtime_capture_validation_matrix['promotionBlockers'])
        self.assertIn('not-promoted', runtime_capture_validation_matrix['promotionStatus'])
        runtime_capture_rejection_taxonomy = data['nonTopologySystRuntimeCaptureRejectionTaxonomySummary']
        self.assertEqual(runtime_capture_rejection_taxonomy['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-rejection-taxonomy')
        self.assertEqual(runtime_capture_rejection_taxonomy['oracleStatus'], 'non_topology_syst_capture_rejection_taxonomy_blocked_pending_real_control_packets')
        self.assertEqual(runtime_capture_rejection_taxonomy['recordCount'], 67)
        self.assertEqual(runtime_capture_rejection_taxonomy['evidenceInputSummaryCount'], 4)
        self.assertIn('nonTopologySystRuntimeCaptureValidationMatrixSummary', runtime_capture_rejection_taxonomy['evidenceInputSummaries'])
        self.assertEqual(runtime_capture_rejection_taxonomy['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_capture_rejection_taxonomy['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', runtime_capture_rejection_taxonomy['requiredCapturePacketIds'])
        self.assertIn('runtime-observation-used-as-offset-oracle', runtime_capture_rejection_taxonomy['validationFailureClassIds'])
        self.assertEqual(runtime_capture_rejection_taxonomy['rejectionClassCount'], 4)
        self.assertIn('topology-boundary-contamination', runtime_capture_rejection_taxonomy['rejectionClassIds'])
        self.assertIn('block-promotion-and-preserve-as-priority-refinement-only', runtime_capture_rejection_taxonomy['rejectionDispositions'])
        self.assertIn('assigning government, message, hazard, interference, visibility, or port offsets from rejected runtime observations', runtime_capture_rejection_taxonomy['blockedPromotionClaims'])
        self.assertIn('rejection taxonomy is a failure-routing checklist, not Classic byte-offset evidence', runtime_capture_rejection_taxonomy['promotionBlockers'])
        self.assertIn('not-promoted', runtime_capture_rejection_taxonomy['promotionStatus'])
        runtime_capture_reentry_guardrail = data['nonTopologySystRuntimeCaptureReentryGuardrailSummary']
        self.assertEqual(runtime_capture_reentry_guardrail['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-reentry-guardrail')
        self.assertEqual(runtime_capture_reentry_guardrail['oracleStatus'], 'non_topology_syst_capture_reentry_blocked_pending_corrected_control_packets')
        self.assertEqual(runtime_capture_reentry_guardrail['recordCount'], 67)
        self.assertEqual(runtime_capture_reentry_guardrail['evidenceInputSummaryCount'], 4)
        self.assertIn('nonTopologySystRuntimeCaptureRejectionTaxonomySummary', runtime_capture_reentry_guardrail['evidenceInputSummaries'])
        self.assertEqual(runtime_capture_reentry_guardrail['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_capture_reentry_guardrail['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', runtime_capture_reentry_guardrail['requiredCapturePacketIds'])
        self.assertEqual(runtime_capture_reentry_guardrail['reentryStepCount'], 4)
        self.assertIn('topology-boundary-redaction', runtime_capture_reentry_guardrail['reentryStepIds'])
        self.assertIn('runtime-observation-used-as-offset-oracle', runtime_capture_reentry_guardrail['rejectionClassIds'])
        self.assertIn('allowing rejected non-topology capture packets back into reconciliation without corrected provenance and control-pair evidence', runtime_capture_reentry_guardrail['blockedPromotionClaims'])
        self.assertIn('reentry guardrail is a replay-path checklist, not Classic byte-offset evidence', runtime_capture_reentry_guardrail['promotionBlockers'])
        self.assertIn('not-promoted', runtime_capture_reentry_guardrail['promotionStatus'])
        field_family_reference = data['nonTopologySystFieldFamilyReferenceSummary']
        self.assertEqual(field_family_reference['sourceLabel'], 'resource-bible-backed-non-topology-syst-field-family-reference')
        self.assertEqual(field_family_reference['oracleStatus'], 'non_topology_syst_field_families_blocked_pending_syst_template_offset_oracle')
        self.assertEqual(field_family_reference['recordCount'], 67)
        self.assertEqual(field_family_reference['recordSize'], 88)
        self.assertEqual(field_family_reference['fieldFamilyCount'], 8)
        self.assertEqual(field_family_reference['individualFieldCount'], 29)
        self.assertEqual(field_family_reference['familyIds'], ['navDef', 'dudeTypesAndProbability', 'govt', 'messageBuoy', 'asteroids', 'interference', 'visBit', 'con6ToCon16'])
        first_family = field_family_reference['fieldFamilies'][0]
        self.assertEqual(first_family['familyId'], 'navDef')
        self.assertEqual(first_family['familyName'], 'NavDef F1-F4 Navigation Defaults')
        self.assertEqual(first_family['fieldCount'], 4)
        self.assertEqual(first_family['candidateWordIndices'], [9, 10, 11, 12])
        self.assertEqual(first_family['promotionStatus'], 'not-promoted; offset oracle gap blocks assignment of decoded words to NavDef fields')
        last_family = field_family_reference['fieldFamilies'][-1]
        self.assertEqual(last_family['familyId'], 'con6ToCon16')
        self.assertEqual(last_family['fieldCount'], 11)
        self.assertEqual(last_family['candidateWordIndices'], list(range(27, 38)))
        self.assertIn('Classic-specific syst TMPL/ResEdit/source template-offset evidence is absent', field_family_reference['promotionBlockers'])
        self.assertIn('not-promoted', field_family_reference['promotionStatus'])
        runtime_capture_reconciliation = data['nonTopologySystRuntimeCaptureReconciliationSummary']
        self.assertEqual(runtime_capture_reconciliation['sourceLabel'], 'decoded-resource-backed-non-topology-syst-runtime-capture-reconciliation-plan')
        self.assertEqual(runtime_capture_reconciliation['oracleStatus'], 'non_topology_syst_capture_reconciliation_blocked_pending_validated_runtime_packets')
        self.assertEqual(runtime_capture_reconciliation['recordCount'], 67)
        self.assertEqual(runtime_capture_reconciliation['evidenceInputSummaryCount'], 5)
        self.assertIn('nonTopologySystRuntimeCaptureGateSummary', runtime_capture_reconciliation['evidenceInputSummaries'])
        self.assertEqual(runtime_capture_reconciliation['firstPriorityProbeFamily'], 'government-message-hazard-visibility-disambiguation')
        self.assertEqual(runtime_capture_reconciliation['firstPriorityCandidateWordIndices'], [22, 23, 24, 25, 26])
        self.assertEqual(runtime_capture_reconciliation['requiredValidatedCapturePacketCount'], 2)
        self.assertIn('non-topology-gov-message-hazard-visibility-positive-control', runtime_capture_reconciliation['requiredCapturePacketIds'])
        self.assertIn('validate every required positive and negative control packet against the capture gate schema', runtime_capture_reconciliation['postCaptureReconciliationSteps'])
        self.assertIn('priority-refined; capture narrows the next source/runtime query but offsets remain unresolved', runtime_capture_reconciliation['promotionDecisionStates'])
        self.assertIn('assigning Govt/Message/Asteroids/Interference/VisBit offsets from runtime observation alone', runtime_capture_reconciliation['blockedPromotionClaims'])
        self.assertIn('post-capture reconciliation is a gate, not a promotion packet', runtime_capture_reconciliation['promotionBlockers'])
        self.assertIn('not-promoted', runtime_capture_reconciliation['promotionStatus'])
        ...
        self.assertEqual(data['fieldFamilies']['mapCoordinates']['resourceBibleFieldFamily'], 'syst xPos/yPos')
        self.assertEqual(first['semanticFields']['mapCoordinates']['xPos']['wordIndices'], [0, 1])
        self.assertEqual(first['semanticFields']['mapCoordinates']['yPos']['wordIndices'], [2, 3])
        self.assertEqual(first['semanticFields']['mapCoordinates']['xPos']['rawWords'], [1, 128])
        self.assertEqual(first['semanticFields']['mapCoordinates']['yPos']['rawWords'], [127, 4096])
        self.assertEqual(first['semanticFields']['mapCoordinates']['xPos']['signedLongCandidate'], 65664)
        ...
        display_bounds = data['coordinateDisplayBoundsSummary']
        self.assertEqual(display_bounds['sourceLabel'], 'decoded-resource-backed-coordinate-display-bounds-scout')
        self.assertEqual(display_bounds['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(display_bounds['recordCount'], 67)
        self.assertIn('raw high-word candidate bounds/span', display_bounds['candidateFamilies'])
        self.assertEqual(display_bounds['xPos']['rawHighWordCandidateSpan'], 3)
        self.assertEqual(display_bounds['xPos']['rawLowWordCandidateSpan'], 153)
        self.assertEqual(display_bounds['xPos']['signedLongCandidateSpan'], 262015)
        ...
        display_normalized = data['coordinateDisplayNormalizedSummary']
        self.assertEqual(display_normalized['sourceLabel'], 'decoded-resource-backed-coordinate-display-normalized-scout')
        self.assertEqual(display_normalized['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(display_normalized['recordCount'], 67)
        self.assertIn('signed-long min-normalized x/y candidates', display_normalized['candidateFamilies'])
        self.assertEqual(display_normalized['xPos']['signedLongCandidateMinMax'], [65664, 327679])
        self.assertEqual(display_normalized['xPos']['minNormalizedSignedLongCandidateRange'], [0, 262015])
        self.assertEqual(display_normalized['yPos']['signedLongCandidateMinMax'], [1, 8720384])
        self.assertEqual(display_normalized['yPos']['minNormalizedSignedLongCandidateRange'], [0, 8720383])
        self.assertEqual(display_normalized['resource128']['xPos']['minNormalizedSignedLongCandidate'], 0)
        self.assertEqual(display_normalized['resource128']['yPos']['minNormalizedSignedLongCandidate'], 8327167)
        self.assertEqual(display_normalized['resource128']['yPos']['unitIntervalCandidate'], 0.954908)
        self.assertEqual(len(display_normalized['perResource']), 67)
        self.assertEqual(display_normalized['perResource'][0], display_normalized['resource128'])
        self.assertEqual(display_normalized['perResource'][1]['resourceId'], 129)
        self.assertEqual(display_normalized['perResource'][1]['xPos']['unitIntervalCandidate'], 0.500246)
        self.assertEqual(display_normalized['perResource'][1]['yPos']['unitIntervalCandidate'], 0.003758)
        display_transform = data['coordinateDisplayTransformSummary']
        self.assertEqual(display_transform['sourceLabel'], 'decoded-resource-backed-coordinate-display-transform-scout')
        self.assertEqual(display_transform['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('signed-long y-axis inversion candidate', display_transform['candidateFamilies'])
        self.assertEqual(display_transform['signedLongAxisSpanRatioYOverX'], 33.281999)
        self.assertEqual(display_transform['resource128']['xPos']['unitIntervalCandidate'], 0)
        ...
        display_integer_band = data['coordinateDisplayIntegerBandSummary']
        self.assertEqual(display_integer_band['sourceLabel'], 'decoded-resource-backed-coordinate-display-integer-band-scout')
        self.assertEqual(display_integer_band['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('16.16 high-word integer-band candidate', display_integer_band['candidateFamilies'])
        self.assertEqual(display_integer_band['xPos']['integerBandCandidateDistribution'], {'1': 12, '2': 7, '3': 32, '4': 16})
        self.assertEqual(display_integer_band['yPos']['integerBandCandidateDistribution'], {'0': 42, '18': 1, '72': 4, '127': 19, '133': 1})
        self.assertEqual(display_integer_band['resource128']['yPos']['integerBandCandidate'], 127)
        self.assertEqual(display_integer_band['resource129']['yPos']['signedFractionalResidualCandidate'], -32768)
        self.assertIn('not-promoted', display_integer_band['displayUnitInterpretationStatus'])
        display_residual_sign = data['coordinateDisplayResidualSignSummary']
        self.assertEqual(display_residual_sign['sourceLabel'], 'decoded-resource-backed-coordinate-display-residual-sign-scout')
        self.assertEqual(display_residual_sign['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('16.16 low-word residual sign distribution candidate', display_residual_sign['candidateFamilies'])
        self.assertEqual(display_residual_sign['fixedPointDivisorCandidate'], 65536)
        self.assertEqual(display_residual_sign['xPos']['signedFractionalResidualSignDistribution'], {'negative': 4, 'zero': 0, 'positive': 63})
        self.assertEqual(display_residual_sign['yPos']['signedFractionalResidualSignDistribution'], {'negative': 54, 'zero': 0, 'positive': 13})
        self.assertEqual(display_residual_sign['yPos']['fractionalUnitCandidateDistinctValues'], [-0.5, 1.5e-05, 0.0625, 0.125, 0.4375])
        self.assertEqual(display_residual_sign['resource128']['yPosFractionalUnitCandidate'], 0.0625)
        self.assertEqual(display_residual_sign['resource129']['yPosFractionalUnitCandidate'], -0.5)
        self.assertIn('not-promoted', display_residual_sign['displayUnitInterpretationStatus'])
        display_residual_magnitude = data['coordinateDisplayResidualMagnitudeSummary']
        self.assertEqual(display_residual_magnitude['sourceLabel'], 'decoded-resource-backed-coordinate-display-residual-magnitude-scout')
        self.assertEqual(display_residual_magnitude['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('16.16 low-word absolute residual magnitude candidate', display_residual_magnitude['candidateFamilies'])
        self.assertEqual(display_residual_magnitude['fixedPointDivisorCandidate'], 65536)
        self.assertEqual(display_residual_magnitude['xPos']['absoluteResidualCandidateRange'], [1, 152])
        self.assertEqual(display_residual_magnitude['xPos']['absoluteResidualCandidateDistribution']['128'], 14)
        self.assertEqual(display_residual_magnitude['yPos']['absoluteResidualCandidateRange'], [1, 32768])
        self.assertEqual(display_residual_magnitude['yPos']['absoluteFractionalUnitCandidateDistinctValues'], [1.5e-05, 0.0625, 0.125, 0.4375, 0.5])
        self.assertEqual(display_residual_magnitude['yPos']['maxResidualMagnitudeResourceIds'][:3], [129, 130, 132])
        self.assertEqual(display_residual_magnitude['resource128']['yPosAbsoluteFractionalUnitCandidate'], 0.0625)
        self.assertEqual(display_residual_magnitude['resource129']['yPosAbsoluteFractionalUnitCandidate'], 0.5)
        self.assertIn('not-promoted', display_residual_magnitude['displayUnitInterpretationStatus'])
        display_quantization = data['coordinateDisplayQuantizationSummary']
        self.assertEqual(display_quantization['sourceLabel'], 'decoded-resource-backed-coordinate-display-quantization-scout')
        self.assertEqual(display_quantization['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('16.16 residual gcd quantization candidate', display_quantization['candidateFamilies'])
        self.assertEqual(display_quantization['fixedPointDivisorCandidate'], 65536)
        self.assertEqual(display_quantization['xPos']['absoluteResidualGcdCandidate'], 1)
        self.assertEqual(display_quantization['xPos']['residualModulo128Distribution']['0'], 14)
        self.assertEqual(display_quantization['yPos']['coarseGridStepCandidate'], 4096)
        self.assertEqual(display_quantization['yPos']['coarseGridStepFractionalUnitCandidate'], 0.0625)
        ...
        display_calibration_gate = data['coordinateDisplayCalibrationGateSummary']
        self.assertEqual(display_calibration_gate['sourceLabel'], 'decoded-resource-backed-coordinate-display-calibration-gate')
        self.assertEqual(display_calibration_gate['oracleStatus'], 'coordinate_display_calibration_blocked_pending_classic_map_evidence')
        self.assertEqual(display_calibration_gate['recordCount'], 67)
        self.assertEqual(display_calibration_gate['candidateInputSummaryCount'], 8)
        self.assertIn('coordinateDisplayScaleInterpretationSummary', display_calibration_gate['candidateInputSummaries'])
        self.assertIn('coordinateDisplayQuantizationSummary', display_calibration_gate['candidateInputSummaries'])
        self.assertEqual(display_calibration_gate['calibrationRequiredClaims'], [
        ...
        display_runtime_capture_reconciliation = data['coordinateDisplayRuntimeCaptureReconciliationSummary']
        self.assertEqual(display_runtime_capture_reconciliation['sourceLabel'], 'decoded-resource-backed-coordinate-display-runtime-capture-reconciliation-plan')
        self.assertEqual(display_runtime_capture_reconciliation['oracleStatus'], 'coordinate_display_runtime_capture_reconciliation_blocked_pending_validated_map_packets')
        self.assertEqual(display_runtime_capture_reconciliation['recordCount'], 67)
        self.assertEqual(display_runtime_capture_reconciliation['evidenceInputSummaryCount'], 4)
        self.assertEqual(display_runtime_capture_reconciliation['requiredValidatedCapturePacketCount'], 3)
        self.assertEqual(display_runtime_capture_reconciliation['requiredCapturePacketTargetResourceIds'], [129, 130, 131])
        self.assertEqual(display_runtime_capture_reconciliation['requiredCapturePacketConSlots'], ['Con2', 'Con3', 'Con4'])
        self.assertIn('coordinate-display-map-calibration-con2-resource-129', display_runtime_capture_reconciliation['requiredCapturePacketIds'])
        self.assertIn('validate every required map calibration packet against the coordinate display runtime capture gate schema', display_runtime_capture_reconciliation['postCaptureReconciliationSteps'])
        self.assertIn('source-runtime-corroborated; may propose a narrow coordinate display calibration packet but not broad universe replacement', display_runtime_capture_reconciliation['promotionDecisionStates'])
        self.assertIn('assigning Classic map pixel scale from static coordinate candidates alone', display_runtime_capture_reconciliation['blockedPromotionClaims'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', display_runtime_capture_reconciliation['requiredVerifierBeforeGameplay'])
        self.assertIn('post-capture reconciliation is a gate, not a display-unit promotion packet', display_runtime_capture_reconciliation['promotionBlockers'])
        self.assertIn('not-promoted', display_runtime_capture_reconciliation['promotionStatus'])
        display_runtime_capture_validation = data['coordinateDisplayRuntimeCaptureValidationMatrixSummary']
        self.assertEqual(display_runtime_capture_validation['sourceLabel'], 'original-runtime-coordinate-display-capture-validation-matrix')
        self.assertEqual(display_runtime_capture_validation['oracleStatus'], 'coordinate_display_capture_validation_blocked_pending_real_map_packets')
        self.assertEqual(display_runtime_capture_validation['recordCount'], 67)
        self.assertEqual(display_runtime_capture_validation['evidenceInputSummaryCount'], 4)
        self.assertEqual(display_runtime_capture_validation['requiredCapturePacketTargetResourceIds'], [129, 130, 131])
        self.assertEqual(display_runtime_capture_validation['requiredCapturePacketConSlots'], ['Con2', 'Con3', 'Con4'])
        self.assertEqual(display_runtime_capture_validation['validationCheckCount'], 5)
        self.assertIn('map-pixel-observation-completeness', display_runtime_capture_validation['validationCheckIds'])
        self.assertEqual(display_runtime_capture_validation['failureClassCount'], 4)
        self.assertIn('promotion-boundary-contamination', display_runtime_capture_validation['failureClassIds'])
        self.assertEqual(display_runtime_capture_validation['minimumAcceptedPacketCountBeforeScaleReview'], 2)
        self.assertIn('using a single named-label observation to promote Classic map scale, projection, origin, centering, or y-axis orientation', display_runtime_capture_validation['blockedPromotionClaims'])
        self.assertIn('validation matrix is an acceptance schema, not Classic map display evidence', display_runtime_capture_validation['promotionBlockers'])
        self.assertIn('not-promoted', display_runtime_capture_validation['promotionStatus'])
        ...
        self.assertEqual(seed_summary['systemNameSeedCount'], 9)
        self.assertEqual(seed_summary['systemNameSeedNames'][:3], ['Sol', 'Centauri', 'Sirius'])
        ...
        self.assertEqual(record_name_readiness['recordCount'], 67)
        self.assertEqual(record_name_readiness['exactMappedRecordCount'], 1)
        self.assertEqual(record_name_readiness['exactMappedResourceIds'], [128])
        self.assertEqual(record_name_readiness['exactMappedSystemNames'], ['Levo'])
        self.assertEqual(record_name_readiness['unjoinedRecordCount'], 66)
        self.assertEqual(record_name_readiness['unjoinedResourceIdRange'], [129, 194])
        self.assertEqual(record_name_readiness['heuristicSystemNameSeedCount'], 9)
        ...
        record_name_oracle = data['recordNameOracleEvidenceMatrixSummary']
        self.assertEqual(record_name_oracle['sourceLabel'], 'decoded-resource-backed-record-name-oracle-evidence-matrix')
        self.assertEqual(record_name_oracle['oracleStatus'], 'record_name_mapping_blocked_pending_complete_name_order_or_runtime_label_oracle')
        self.assertEqual(record_name_oracle['recordCount'], 67)
        self.assertEqual(record_name_oracle['evidenceInputSummaryCount'], 5)
        self.assertIn('recordToNamePromotionReadinessSummary', record_name_oracle['evidenceInputSummaries'])
        self.assertEqual(record_name_oracle['exactMappedResourceIds'], [128])
        self.assertEqual(record_name_oracle['exactMappedSystemNames'], ['Levo'])
        self.assertEqual(record_name_oracle['unjoinedRecordCount'], 66)
        self.assertEqual(record_name_oracle['unjoinedResourceIdRange'], [129, 194])
        self.assertEqual(record_name_oracle['heuristicSystemNameSeedCount'], 9)
        self.assertGreaterEqual(record_name_oracle['landingNameSeedCount'], 70)
        self.assertIn('complete source/runtime name ordering', record_name_oracle['requiredOracleClaims'][0])
        ...
        route_label_probe_targeting = data['runtimeRouteLabelProbeTargetingSummary']
        self.assertEqual(route_label_probe_targeting['sourceLabel'], 'original-runtime-and-decoded-resource-backed-route-label-probe-targeting-matrix')
        self.assertEqual(route_label_probe_targeting['oracleStatus'], 'route_label_probe_targets_pending_decoded_resource_id_slot_join')
        self.assertIn('original-runtime-observed', route_label_probe_targeting['sourceBasis'])
        self.assertIn('decoded-original-variable', route_label_probe_targeting['sourceBasis'])
        self.assertEqual(route_label_probe_targeting['candidateStartResourceId'], 128)
        self.assertEqual(route_label_probe_targeting['observedRuntimeLabelCount'], 4)
        self.assertEqual(route_label_probe_targeting['unjoinedStartNeighborResourceIds'], [129, 130, 131])
        self.assertEqual(route_label_probe_targeting['probeTargetCount'], 3)
        self.assertEqual([entry['slotName'] for entry in route_label_probe_targeting['probeTargets']], ['Con2', 'Con3', 'Con4'])
        ...
        route_label_probe_priority = data['runtimeRouteLabelProbePrioritySummary']
        self.assertEqual(route_label_probe_priority['sourceLabel'], 'decoded-resource-and-runtime-gate-backed-route-label-probe-priority')
        self.assertEqual(route_label_probe_priority['oracleStatus'], 'route_label_probe_priority_pending_disposable_runtime_capture')
        self.assertEqual(route_label_probe_priority['requiredProbeTargetResourceIds'], [129, 130, 131])
        self.assertEqual(route_label_probe_priority['displayCalibrationPriorityTargetResourceIds'], [131, 129, 130])
        self.assertEqual(route_label_probe_priority['priorityTargetResourceIds'], [131, 129, 130])
        self.assertEqual(route_label_probe_priority['priorityConSlots'], ['Con4', 'Con2', 'Con3'])
        self.assertEqual(route_label_probe_priority['firstPriorityTargetResourceId'], 131)
        self.assertEqual(route_label_probe_priority['firstPriorityConSlot'], 'Con4')
        self.assertEqual(route_label_probe_priority['probePriorityEntries'][0]['priorityReason'], 'nearest-and-lowest-angle-candidate')
        self.assertIn('observed runtime labels remain unassigned to resource IDs 129-131 until disposable capture packets are executed', route_label_probe_priority['promotionBlockers'])
        self.assertIn('not-promoted', route_label_probe_priority['promotionStatus'])
        route_label_capture_reconciliation = data['runtimeRouteLabelCaptureReconciliationSummary']
        self.assertEqual(route_label_capture_reconciliation['sourceLabel'], 'decoded-resource-and-runtime-backed-route-label-capture-reconciliation-plan')
        self.assertEqual(route_label_capture_reconciliation['oracleStatus'], 'route_label_capture_reconciliation_blocked_pending_validated_probe_packets')
        self.assertEqual(route_label_capture_reconciliation['sourceBasis'], ['original-runtime-observed', 'decoded-original-variable', 'resource-bible-field', 'original-runtime-required'])
        self.assertEqual(route_label_capture_reconciliation['candidateStartResourceId'], 128)
        self.assertEqual(route_label_capture_reconciliation['candidateStartSystemName'], 'Levo')
        self.assertEqual(route_label_capture_reconciliation['observedRuntimeLabelCount'], 4)
        self.assertEqual(route_label_capture_reconciliation['observedRuntimeLabels'], ['Rigel', 'Kathoon', 'Yemuro', 'Torgo'])
        self.assertEqual(route_label_capture_reconciliation['requiredProbeTargetResourceIds'], [129, 130, 131])
        self.assertEqual(route_label_capture_reconciliation['requiredProbeConSlots'], ['Con2', 'Con3', 'Con4'])
        self.assertEqual(route_label_capture_reconciliation['priorityTargetResourceIds'], [131, 129, 130])
        self.assertEqual(route_label_capture_reconciliation['priorityConSlots'], ['Con4', 'Con2', 'Con3'])
        self.assertEqual(route_label_capture_reconciliation['requiredValidatedCapturePacketCount'], 3)
        self.assertIn('route-label-con4-resource-131', route_label_capture_reconciliation['requiredCapturePacketIds'])
        self.assertEqual(route_label_capture_reconciliation['evidenceInputSummaryCount'], 5)
        self.assertIn('coordinateDisplayRuntimeCaptureReconciliationSummary', route_label_capture_reconciliation['evidenceInputSummaries'])
        self.assertIn('validate every route-label packet against the execution gate schema before considering any resource-ID/name join', route_label_capture_reconciliation['postCaptureReconciliationSteps'])
        self.assertIn('partial-packets; route-label capture worklist remains open and no named topology may be promoted', route_label_capture_reconciliation['promotionDecisionStates'])
        self.assertIn('assigning Rigel, Kathoon, Yemuro, or Torgo to decoded resource IDs 129-131 without validated target packets', route_label_capture_reconciliation['blockedPromotionClaims'])
        self.assertIn('python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty', route_label_capture_reconciliation['requiredVerifierBeforeGameplay'])
        self.assertIn('post-capture reconciliation is a gate, not a route-label promotion packet', route_label_capture_reconciliation['promotionBlockers'])
        self.assertIn('not-promoted', route_label_capture_reconciliation['promotionStatus'])
        ...
        self.assertIn('~51 distinct record-to-name joins', str(topology_readiness['blockedPromotionClaims']))
        self.assertEqual(topology_readiness.get('deduplicationAdjustedDistinctSystemCountEstimate'), 51)
        ...
        runtime_gate = data['runtimeUniverseReplacementGateSummary']
        self.assertEqual(runtime_gate['sourceLabel'], 'decoded-resource-backed-runtime-universe-replacement-gate')
        self.assertEqual(runtime_gate['oracleStatus'], 'runtime_universe_replacement_blocked_pending_name_topology_display_oracles')
        self.assertEqual(runtime_gate['decodedSystRecordCount'], 67)
        self.assertEqual(runtime_gate['exactMappedResourceIds'], [128])
        self.assertEqual(runtime_gate['unjoinedRecordCount'], 66)
        self.assertEqual(runtime_gate['unjoinedResourceIdRange'], [129, 194])
        gate_names = [entry['gate'] for entry in runtime_gate['blockingGates']]
        self.assertIn('coordinate_display_units_map_scaling_projection', gate_names)
        self.assertIn('remaining_record_to_name_joins', gate_names)
        self.assertIn('named_route_topology_labels', gate_names)
        self.assertIn('non_topology_syst_field_semantics', gate_names)
        self.assertIn('not-ready', runtime_gate['replacementReadinessStatus'])
        self.assertEqual(runtime_gate['evidenceInputSummaryCount'], 6)
        self.assertIn('recordNameRuntimeJoinReconciliationSummary', runtime_gate['evidenceInputSummaries'])
        self.assertIn('coordinateDisplayRuntimeCaptureReconciliationSummary', runtime_gate['evidenceInputSummaries'])
        self.assertIn('nonTopologySystRuntimeCaptureReconciliationSummary', runtime_gate['evidenceInputSummaries'])
        self.assertIn('systTemplateOffsetOracleGapSummary', runtime_gate['evidenceInputSummaries'])
        self.assertIn('syst_template_offset_oracle_gap', gate_names)
        self.assertIn('record-name joins closed by complete source/name table or validated packet-level target evidence', runtime_gate['gateClosureChecklist'])
        self.assertIn('syst template/offset gap closed by Classic-specific TMPL/ResEdit/source evidence before assigning unresolved byte offsets or Con6-Con16 split placement', runtime_gate['gateClosureChecklist'])
        self.assertIn('do not infer Classic-faithful broad universe behavior from the decoded syst run alone', runtime_gate['promotionBlockers'])
        self.assertIn('do not convert Resource Bible syst prose into byte offsets or Con6-Con16 split placement without a Classic-specific template/source oracle', runtime_gate['promotionBlockers'])
        ...
        self.assertIn('system-name text seed to nearest landing-name text seed byte-proximity candidates', landing_proximity['candidateFamilies'])
        self.assertEqual(landing_proximity['systemNameSeedCount'], 9)
        self.assertGreaterEqual(landing_proximity['landingNameSeedCount'], 70)
        self.assertEqual(landing_proximity['systemNameSeedsWithCloseLandingCandidates'], ['Centauri', 'Sirius', 'Tau Ceti', 'Alkaid', 'Zaxted', 'Clotho'])
        self.assertEqual(landing_proximity['systemNameSeedsWithoutCloseLandingCandidates'], ['Sol', 'Enyo', 'Antares'])
        self.assertEqual(landing_proximity['exactMappedSystemLandingCandidates'], [{'systemName': 'Levo', 'landingNameSeedByteOffsets': [23867]}])
        self.assertEqual(landing_proximity['bySystemNameSeed'][1]['nearestLandingNameCandidates'][0], {'landingName': 'Capella', 'landingByteOffset': 24338, 'byteDeltaFromSystemNameSeed': -77})
        self.assertIn('does not assign any syst resource ID', landing_proximity['sourceNote'])
        landing_name_candidate_ref = data['landingNameCandidateReferenceSummary']
        self.assertEqual(landing_name_candidate_ref['sourceLabel'], 'decoded-resource-backed-landing-name-candidate-reference')
        self.assertEqual(landing_name_candidate_ref['oracleStatus'], 'exact_record_name_runtime_topology_mapping_pending')
        self.assertGreaterEqual(landing_name_candidate_ref['recordCount'], 70)
        self.assertLessEqual(landing_name_candidate_ref['recordCount'], 80)
        self.assertGreater(landing_name_candidate_ref['chunkIndexRange'][1], landing_name_candidate_ref['chunkIndexRange'][0])
        self.assertGreater(landing_name_candidate_ref['chunkIndexCount'], 60)
        self.assertEqual(landing_name_candidate_ref['systemNameSeedMatchCount'], 0)
        self.assertEqual(landing_name_candidate_ref['systemNameSeedMatches'], [])
        self.assertEqual(landing_name_candidate_ref['exactSystemNameMappingMatchCount'], 1)
        self.assertEqual(landing_name_candidate_ref['exactSystemNameMappingMatches'], ['Levo'])
        self.assertGreaterEqual(len(landing_name_candidate_ref['candidateNames']), 70)
        first_candidate = landing_name_candidate_ref['candidateNames'][0]
        self.assertIn('name', first_candidate)
        self.assertIn('chunkIndex', first_candidate)
        self.assertEqual(first_candidate['isSystemNameSeed'], False)
        self.assertEqual(first_candidate['isExactSystemNameMapping'], False)
        self.assertIn('landing-name chunkIndex ordering may correspond to EV Data.rez system resource ordering', landing_name_candidate_ref['candidateHypothesis'])
        self.assertIn('landing-name chunk ordering is a heuristic signal, not a validated system resource ordering oracle', landing_name_candidate_ref['promotionBlockers'])
        self.assertIn('not-promoted', landing_name_candidate_ref['promotionStatus'])
        self.assertIn('72 landing-name seeds', landing_name_candidate_ref['sourceNote'])
        link_graph = data['candidateLinkGraphSummary']
        self.assertEqual(link_graph['sourceLabel'], 'decoded-resource-backed-candidate-link-graph-scout')
        self.assertEqual(link_graph['oracleStatus'], 'exact_record_name_runtime_topology_mapping_pending')
        self.assertEqual(link_graph['recordCount'], 67)
        self.assertEqual(link_graph['directedLinkSlotCount'], 268)
        self.assertEqual(link_graph['uniqueDirectedLinkCount'], 117)
        self.assertEqual(link_graph['reciprocalDirectedLinkCount'], 8)
        self.assertEqual(link_graph['nonReciprocalDirectedLinkCount'], 109)
        self.assertEqual(link_graph['nonReciprocalDirectedLinkSample'][:3], [[128, 129], [128, 130], [128, 131]])
        self.assertEqual(link_graph['linkedSlotsPerSystemRange'], [4, 4])
        self.assertEqual(link_graph['systemsWithNoLinkedSlots'], 0)
        self.assertTrue(link_graph['allTargetsPresentInSystRun'])
        self.assertEqual(link_graph['missingTargetEdges'], [])
        self.assertEqual(link_graph['selfLinkSlotCount'], 7)
        ...
        start_display = data['startNeighborhoodDisplayTransformSummary']
        self.assertEqual(start_display['sourceLabel'], 'decoded-resource-backed-start-neighborhood-display-transform-scout')
        self.assertEqual(start_display['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(start_display['startResourceId'], 128)
        self.assertEqual(start_display['startExactSystemName'], 'Levo')
        self.assertIn('start-neighborhood inverted-y display-transform candidates', start_display['candidateFamilies'])
        self.assertEqual(start_display['startUnitIntervalCandidate'], {'xPos': 0, 'yPos': 0.954908, 'invertedYPos': 0.045092})
        self.assertEqual(start_display['linkedNeighborCount'], 4)
        self.assertEqual([neighbor['targetResourceId'] for neighbor in start_display['linkedNeighbors']], [128, 129, 130, 131])
        self.assertEqual(start_display['linkedNeighbors'][1]['deltaSignedLongFromStart'], {'xPos': 131072, 'yPos': -8294400})
        self.assertEqual(start_display['linkedNeighbors'][1]['unitIntervalCandidate'], {'xPos': 0.500246, 'yPos': 0.003758, 'invertedYPos': 0.996242})
        self.assertEqual(start_display['linkedNeighbors'][3]['deltaSignedLongFromStart'], {'xPos': 65536, 'yPos': 4096})
        self.assertEqual(start_display['linkedNeighbors'][3]['unitIntervalCandidate']['invertedYPos'], 0.044622)
        self.assertEqual(start_display['unjoinedNeighborResourceIds'], [129, 130, 131])
        start_distance = data['startNeighborhoodDisplayDistanceSummary']
        self.assertEqual(start_distance['sourceLabel'], 'decoded-resource-backed-start-neighborhood-display-distance-scout')
        self.assertEqual(start_distance['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertEqual(start_distance['startResourceId'], 128)
        self.assertEqual(start_distance['startExactSystemName'], 'Levo')
        self.assertIn('start-neighborhood inverted-y unit-interval manhattan-distance candidates', start_distance['candidateFamilies'])
        self.assertEqual(start_distance['linkedNeighborCount'], 4)
        self.assertEqual([neighbor['targetResourceId'] for neighbor in start_distance['linkedNeighbors']], [128, 129, 130, 131])
        self.assertEqual(start_distance['linkedNeighbors'][1]['manhattanSignedLongCandidate'], 8425472)
        self.assertEqual(start_distance['linkedNeighbors'][1]['deltaUnitIntervalCandidate'], {'xPos': 0.500246, 'yPos': -0.95115, 'invertedYPos': 0.95115})
        self.assertEqual(start_distance['linkedNeighbors'][3]['manhattanInvertedYUnitIntervalCandidate'], 0.250593)
        self.assertEqual(start_distance['nonSelfSignedLongManhattanDistanceRange'], [69632, 8425473])
        self.assertEqual(start_distance['nonSelfInvertedYUnitIntervalManhattanDistanceRange'], [0.250593, 1.4514])
        start_vector = data['startNeighborhoodDisplayVectorSummary']
        self.assertEqual(start_vector['sourceLabel'], 'decoded-resource-backed-start-neighborhood-display-vector-scout')
        self.assertEqual(start_vector['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('start-neighborhood display quadrant candidates', start_vector['candidateFamilies'])
        self.assertEqual(start_vector['linkedNeighborCount'], 4)
        self.assertEqual([neighbor['targetResourceId'] for neighbor in start_vector['linkedNeighbors']], [128, 129, 130, 131])
        self.assertEqual(start_vector['linkedNeighbors'][1]['deltaInvertedYUnitIntervalCandidate'], {'xPos': 0.500246, 'yPos': 0.95115})
        self.assertEqual(start_vector['linkedNeighbors'][1]['displayQuadrantCandidate'], 'north-east')
        self.assertEqual(start_vector['linkedNeighbors'][1]['dominantAxisCandidate'], 'y')
        self.assertEqual(start_vector['linkedNeighbors'][3]['signedAngleDegreesFromPositiveXCandidate'], -0.107663)
        self.assertEqual(start_vector['nonSelfDisplayQuadrantCandidates'], ['north-east', 'south-east'])
        self.assertEqual(start_vector['nonSelfDominantAxisDistribution'], {'x': 1, 'y': 2})
        slot_order = data['startNeighborhoodSlotVectorOrderSummary']
        self.assertEqual(slot_order['sourceLabel'], 'decoded-resource-backed-start-neighborhood-slot-vector-order-scout')
        self.assertEqual(slot_order['oracleStatus'], 'coordinate_display_units_map_scaling_pending')
        self.assertIn('start-neighborhood Con-slot order candidates', slot_order['candidateFamilies'])
        self.assertEqual(slot_order['linkedNeighborCount'], 4)
        self.assertEqual([entry['slotName'] for entry in slot_order['linkedSlotOrder']], ['Con1', 'Con2', 'Con3', 'Con4'])
        self.assertEqual([entry['targetResourceId'] for entry in slot_order['linkedSlotOrder']], [128, 129, 130, 131])
        self.assertEqual(slot_order['linkedSlotOrder'][0]['displayQuadrantCandidate'], 'self')
        self.assertEqual(slot_order['linkedSlotOrder'][3]['distanceRankAmongNonSelfCandidates'], 1)
        self.assertEqual(slot_order['nonSelfSlotNamesByDistanceCandidate'], ['Con4', 'Con2', 'Con3'])
        ...
        calibration_priority = data['startNeighborhoodRuntimeCalibrationPrioritySummary']
        self.assertEqual(calibration_priority['sourceLabel'], 'decoded-resource-backed-start-neighborhood-runtime-calibration-priority-scout')
        self.assertEqual(calibration_priority['oracleStatus'], 'coordinate_display_capture_blocked_pending_classic_map_pixel_click_evidence')
        self.assertIn('runtime calibration target priority candidates', calibration_priority['candidateFamilies'][0])
        self.assertEqual(calibration_priority['requiredRuntimeCaptureTargetResourceIds'], [129, 130, 131])
        self.assertEqual(calibration_priority['requiredRuntimeCaptureConSlots'], ['Con2', 'Con3', 'Con4'])
        self.assertEqual(calibration_priority['priorityTargetResourceIds'], [131, 129, 130])
        self.assertEqual(calibration_priority['priorityConSlots'], ['Con4', 'Con2', 'Con3'])
        self.assertEqual(calibration_priority['firstPriorityTargetResourceId'], 131)
        self.assertEqual(calibration_priority['firstPriorityConSlot'], 'Con4')
        self.assertTrue(calibration_priority['distanceAngleOrderConflict'])
        self.assertEqual(calibration_priority['priorityEntries'][0]['priorityReason'], 'nearest-and-lowest-angle-candidate')
        self.assertEqual(calibration_priority['priorityEntries'][1]['targetNameJoinStatus'], 'unjoined')
        self.assertIn('not-promoted', calibration_priority['promotionStatus'])
        self.assertIn('runtime capture packets must tie visible labels', ' '.join(calibration_priority['promotionBlockers']))
        ...
        self.assertEqual(first_link_slots[0], {
            ...
            'slotName': 'Con1',
            'wordIndex': 4,
            'byteOffsetInRecord': 8,
            'rawValue': 128,
            'status': 'linked-system',
            'targetResourceId': 128,
            'targetOrdinal': 0,
            'targetPresentInSystRun': True,
        })
        self.assertEqual(first_link_slots[4]['slotName'], 'Con5')
        self.assertEqual(first_link_slots[4]['status'], 'no-link')
        ...
        self.assertIn('coordinate display calibration gate matrix', data['promotionBoundary'])
        self.assertIn('non-promoted coordinate scale-interpretation blocker/comparison candidates', data['promotionBoundary'])
        ...
        # Validate systGovtFieldNameCrossReferenceScout
        name_ref = data['systGovtFieldNameCrossReferenceScout']
        self.assertEqual(name_ref['sourceLabel'], 'decoded-resource-backed-syst-govt-field-name-cross-reference-scout')
        self.assertEqual(name_ref['oracleStatus'], 'govt_field_name_cross_reference_partial_matches_non_promoting')
        self.assertEqual(name_ref['recordCount'], 67)
        self.assertEqual(name_ref['matchedCount'] + name_ref['unmatchedCount'], 67,
                         'All 67 systems must be classified as matched or unmatched')
        self.assertIn('not-promoted', name_ref['promotionStatus'])
        self.assertGreater(len(name_ref['matchedGovernmentOrdinalValues']), 0)
        self.assertGreater(len(name_ref['unmatchedRawValues']), 0)
        self.assertIn('not-promoted', name_ref['promotionStatus'])
        self.assertGreater(name_ref['nameDistribution'].get('unmatched_raw_25', 0), 0,
                           'rawValue=25 should be the dominant unmatched value')

        # Validate systGovtFieldResourceIdCrossReferenceScout
        rid_ref = data['systGovtFieldResourceIdCrossReferenceScout']
        self.assertEqual(rid_ref['sourceLabel'], 'decoded-resource-backed-syst-govt-field-resource-id-cross-reference-scout')
        self.assertEqual(rid_ref['oracleStatus'], 'govt_field_resource_id_cross_reference_all_systems_out_of_domain')
        self.assertEqual(rid_ref['recordCount'], 67)
        self.assertTrue(rid_ref['allSystemsOutOfDomain'],
                        'All 67 systems must be classified as out-of-domain')
        self.assertEqual(rid_ref['rawValueRange'], [15, 55])
        self.assertEqual(rid_ref['expectedValueRange'], [128, 255])
        self.assertEqual(rid_ref['resourceIdMatchValues'], [],
                         'No raw values should directly match resource IDs in 128-255 range')
        self.assertEqual(rid_ref['ordinalMatchValues'], [15, 19, 20, 21])
        self.assertIn('not-promoted', rid_ref['promotionStatus'])
        self.assertTrue(rid_ref['allSystemsOutOfDomain'],
    ...
    def test_godot_commodity_buy_blocked_recovery_log_contract(self):
        ...
        for symbol in [
            ...
            '--tv-commodity-buy-blocked-recovery-log',
            'func _run_commodity_buy_blocked_recovery_log() -> void:',
            'inSpaceBuyBlocked=true',
            ...
            'oracleStatus=commodity_buy_blocked_recovery_pending_classic_runtime_trace',
        ]:
            self.assertIn(symbol, main_script)
