...
class TvLedgerReconcileTests(unittest.TestCase):
    ...
    def test_dry_run_reports_stale_projection_without_inventing_live_owner(self):
        ...
        self.assertEqual(payload["recommended_action"], "write_normalized_provenance")
        planned = payload["planned_projection"]
        self.assertNotIn("status", planned)
        self.assertNotIn("declared_owner", planned)
        self.assertNotIn("runner_ownership", planned)
        self.assertNotIn("active_gate", planned)
        self.assertNotIn("last_reconciled_live_git", planned)
        self.assertEqual(planned["runtime_truth_rule"], "Live runner state is derived from Kanban/topology/git/processes, not this ledger.")
        self.assertNotIn("previous_status_removed", planned["historical_notes"])
        self.assertNotIn("previous_declared_owner_removed", planned["historical_notes"])
        self.assertNotIn("removed_live_fields", planned["historical_notes"])
        self.assertEqual(planned["latest_worker_handoff"]["event_id"], "evt-newer")
        self.assertEqual(planned["diagnostics"]["generated_from"]["topology"]["live_implementation_owner"], "none_active")
