...
def _scenario_checks(name: str, state: dict[str, Any], trace: list[dict[str, Any]], all_actions_valid: bool) -> dict[str, str]:
    ...
    if name == 'levo_merchant_first_hop':
        ...
        checks.update({
            ...
            'recorded_topology_promotion_readiness_matrix': 'passed' if readiness.get('topologyPromotionReadinessSourceLabel') == 'decoded-resource-backed-topology-promotion-readiness-matrix' and readiness.get('topologyPromotionReadinessOracleStatus') == 'topology_semantic_promotion_pending_field_family_mapping' and 'complete decoded coordinate word pairs and link slots for all 67 records' in readiness.get('topologyPromotionReadinessReadyStaticInputFamilies', []) and 'remaining 66 exact record-to-name joins' in readiness.get('topologyPromotionReadinessBlockedPromotionClaims', []) and readiness.get('topologyPromotionReadinessExactMappedResourceIds') == [128] and readiness.get('topologyPromotionReadinessUnjoinedRecordCount') == 66 and 'not-promoted' in readiness.get('topologyPromotionReadinessCoordinateStatus', '') and 'not-promoted' in readiness.get('topologyPromotionReadinessRecordNameStatus', '') and 'blocked' in readiness.get('topologyPromotionReadinessRuntimeUniverseReplacementStatus', '') and static_state.get('topologyPromotionReadinessUnjoinedRecordCount') == 66 else 'failed',
