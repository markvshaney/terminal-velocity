...
def sourced_ev_systems_manifest(path=SOURCED_EV_SYSTEMS_PATH):
    ...
    for system in systems:
        ...
        allowed_system_statuses = {
            ...
            'ids_promoted_exact_name_coordinate_words_links_govt_data_word_candidate_fields_pending',
            'ids_promoted_names_seeded_coordinate_words_links_govt_data_word_candidate_fields_pending',
        }
    ...
    width_offset_gap = byte_size_gap.get('fieldWidthOffsetOracleGapSummary', {})
    if width_offset_gap.get('sourceLabel') != 'resource-bible-backed-syst-field-width-offset-oracle-gap':
        raise ValueError('sourced EV systems manifest missing syst field-width/offset oracle gap')
    if width_offset_gap.get('oracleStatus') != 'syst_field_width_offset_mapping_blocked_pending_source_template_or_runtime_oracle':
        raise ValueError('sourced EV systems manifest field-width/offset gap must keep mapping blocked')
    if len(width_offset_gap.get('requiredEvidenceFamilies', [])) != 3:
    ...
    top_level_template_gap = data.get('systTemplateOffsetOracleGapSummary', {})
    if top_level_template_gap.get('sourceLabel') != 'repo-reference-backed-syst-resedit-template-source-availability-gap':
        raise ValueError('sourced EV systems manifest missing top-level syst template/offset oracle gap')
    if top_level_template_gap.get('oracleStatus') != 'syst_template_offset_mapping_blocked_pending_exact_tmpl_or_resedit_source':
        raise ValueError('sourced EV systems manifest top-level template/offset gap must stay blocked pending exact source')
    if top_level_template_gap.get('evidenceInputSummaryCount') != 4:
        raise ValueError('sourced EV systems manifest top-level template/offset gap changed evidence input count')
    if 'resourceBibleSystSequentialFieldProjectionSummary' not in top_level_template_gap.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest top-level template/offset gap missing sequential projection input')
    if 'no exact syst TMPL/resource-template artifact' not in ' '.join(top_level_template_gap.get('missingEvidence', [])):
        raise ValueError('sourced EV systems manifest top-level template/offset gap missing exact TMPL absence evidence')
    if 'template/source absence means Resource Bible prose cannot be converted into byte offsets by assertion' not in top_level_template_gap.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest top-level template/offset gap missing offset assertion blocker')
    if 'not-promoted' not in top_level_template_gap.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest top-level template/offset gap must remain non-promoted')
    template_search_priority = data.get('systTemplateOffsetSourceSearchPrioritySummary', {})
    if template_search_priority.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-source-search-priority':
        raise ValueError('sourced EV systems manifest missing template/offset source search priority')
    if template_search_priority.get('oracleStatus') != 'syst_template_offset_search_blocked_pending_classic_specific_evidence':
        raise ValueError('sourced EV systems manifest template/offset search priority changed oracle status')
    if template_search_priority.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetOracleGapSummary' not in template_search_priority.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset search priority missing evidence inputs')
    if template_search_priority.get('firstPriorityTargetId') != 'classic-syst-tmpl-or-resedit-template':
        raise ValueError('sourced EV systems manifest template/offset search priority changed first target')
    if 'classic-source-syst-struct' not in template_search_priority.get('priorityTargetIds', []):
        raise ValueError('sourced EV systems manifest template/offset search priority missing source struct target')
    if 'search priority is a worklist, not Classic byte-offset evidence' not in template_search_priority.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset search priority missing worklist blocker')
    if 'not-promoted' not in template_search_priority.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset search priority must remain non-promoted')
    template_packet_contract = data.get('systTemplateOffsetEvidencePacketContractSummary', {})
    if template_packet_contract.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-contract':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet contract')
    if template_packet_contract.get('oracleStatus') != 'syst_template_offset_evidence_packets_blocked_pending_validated_classic_specific_packet':
        raise ValueError('sourced EV systems manifest template/offset packet contract changed oracle status')
    if template_packet_contract.get('evidenceInputSummaryCount') != 5 or 'systTemplateOffsetSourceSearchPrioritySummary' not in template_packet_contract.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset packet contract missing evidence inputs')
    if template_packet_contract.get('firstAcceptedEvidenceClassId') != 'classic-syst-tmpl-or-resedit-template':
        raise ValueError('sourced EV systems manifest template/offset packet contract changed first evidence class')
    if 'classic-source-syst-struct' not in template_packet_contract.get('acceptedEvidenceClassIds', []):
        raise ValueError('sourced EV systems manifest template/offset packet contract missing source-struct evidence class')
    if template_packet_contract.get('requiredPacketSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset packet contract changed schema version')
    if 'packet must name Classic-specific provenance before any byte-offset or field-width claim is considered' not in template_packet_contract.get('validationRules', []):
        raise ValueError('sourced EV systems manifest template/offset packet contract missing Classic provenance validation rule')
    if 'evidence packet contract is an acceptance schema, not Classic byte-offset evidence' not in template_packet_contract.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset packet contract missing non-evidence blocker')
    if 'not-promoted' not in template_packet_contract.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset packet contract must remain non-promoted')
    packet_validation_matrix = data.get('systTemplateOffsetEvidencePacketValidationMatrixSummary', {})
    if packet_validation_matrix.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-validation-matrix':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet validation matrix')
    if packet_validation_matrix.get('oracleStatus') != 'syst_template_offset_packet_validation_blocked_pending_real_packet':
        raise ValueError('sourced EV systems manifest template/offset validation matrix changed oracle status')
    if packet_validation_matrix.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketContractSummary' not in packet_validation_matrix.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset validation matrix missing evidence inputs')
    if packet_validation_matrix.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset validation matrix changed contract schema version')
    if packet_validation_matrix.get('validationCaseCount') != 4:
        raise ValueError('sourced EV systems manifest template/offset validation matrix changed case count')
    if packet_validation_matrix.get('firstValidationCaseId') != 'accept-classic-template-field-offset-packet':
        raise ValueError('sourced EV systems manifest template/offset validation matrix changed first case')
    if 'reject-ev-family-template-transfer-packet' not in packet_validation_matrix.get('rejectionCaseIds', []):
        raise ValueError('sourced EV systems manifest template/offset validation matrix missing EV-family rejection case')
    if 'EV-family/adaptation-only packets must be rejected as Classic syst offset evidence' not in packet_validation_matrix.get('requiredVerifierOutcomes', []):
        raise ValueError('sourced EV systems manifest template/offset validation matrix missing EV-family rejection outcome')
    if 'validation matrix is a verifier fixture, not a validated evidence packet' not in packet_validation_matrix.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset validation matrix missing fixture blocker')
    if 'not-promoted' not in packet_validation_matrix.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset validation matrix must remain non-promoted')
    packet_replay_readiness = data.get('systTemplateOffsetEvidencePacketReplayReadinessSummary', {})
    if packet_replay_readiness.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-replay-readiness':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet replay readiness')
    if packet_replay_readiness.get('oracleStatus') != 'syst_template_offset_packet_replay_blocked_pending_real_packet':
        raise ValueError('sourced EV systems manifest template/offset replay readiness changed oracle status')
    if packet_replay_readiness.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketValidationMatrixSummary' not in packet_replay_readiness.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset replay readiness missing evidence inputs')
    if packet_replay_readiness.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset replay readiness changed schema version')
    if packet_replay_readiness.get('replayStepCount') != 4 or packet_replay_readiness.get('firstReplayStepId') != 'packet-artifact-readback':
        raise ValueError('sourced EV systems manifest template/offset replay readiness changed replay steps')
    if 'local-verifier-replay' not in [step.get('stepId') for step in packet_replay_readiness.get('replaySteps', [])]:
        raise ValueError('sourced EV systems manifest template/offset replay readiness missing local verifier step')
    if 'promoting runtime surrogate packets into Classic byte offsets without Classic source/template corroboration' not in packet_replay_readiness.get('blockedPromotionClaims', []):
        raise ValueError('sourced EV systems manifest template/offset replay readiness missing runtime surrogate blocker')
    if 'replay readiness is a handoff checklist, not a validated evidence packet' not in packet_replay_readiness.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset replay readiness missing checklist blocker')
    if 'not-promoted' not in packet_replay_readiness.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset replay readiness must remain non-promoted')
    packet_intake_triage = data.get('systTemplateOffsetEvidencePacketIntakeTriageSummary', {})
    if packet_intake_triage.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-intake-triage':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet intake triage')
    if packet_intake_triage.get('oracleStatus') != 'syst_template_offset_packet_triage_blocked_pending_real_packet':
        raise ValueError('sourced EV systems manifest template/offset intake triage changed oracle status')
    if packet_intake_triage.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketReplayReadinessSummary' not in packet_intake_triage.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset intake triage missing evidence inputs')
    if packet_intake_triage.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset intake triage changed schema version')
    if packet_intake_triage.get('triageDecisionCount') != 4 or 'triage-runtime-surrogate-packet' not in packet_intake_triage.get('triageDecisionIds', []):
        raise ValueError('sourced EV systems manifest template/offset intake triage changed decision set')
    if 'local-verifier-replay' not in packet_intake_triage.get('replayStepIds', []):
        raise ValueError('sourced EV systems manifest template/offset intake triage missing replay step')
    if 'triage-ev-family-or-adaptation-only-packet' not in packet_intake_triage.get('triageDecisionIds', []):
        raise ValueError('sourced EV systems manifest template/offset intake triage missing EV-family rejection triage')
    if 'intake triage is a routing checklist, not a validated evidence packet' not in packet_intake_triage.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset intake triage missing routing-checklist blocker')
    if 'not-promoted' not in packet_intake_triage.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset intake triage must remain non-promoted')
    packet_failure_taxonomy = data.get('systTemplateOffsetEvidencePacketFailureTaxonomySummary', {})
    if packet_failure_taxonomy.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-failure-taxonomy':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet failure taxonomy')
    if packet_failure_taxonomy.get('oracleStatus') != 'syst_template_offset_packet_failure_taxonomy_blocked_pending_real_packet':
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy changed oracle status')
    if packet_failure_taxonomy.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketIntakeTriageSummary' not in packet_failure_taxonomy.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy missing evidence inputs')
    if packet_failure_taxonomy.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy changed schema version')
    if packet_failure_taxonomy.get('failureClassCount') != 5 or 'verifier-replay-missing-or-failed' not in packet_failure_taxonomy.get('failureClassIds', []):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy changed failure classes')
    if 'triage-ev-family-or-adaptation-only-packet' not in packet_failure_taxonomy.get('triageDecisionIds', []):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy missing EV-family triage decision')
    if 'packet verifier output cannot be replayed locally or fails current extractor/model/scenario checks' not in packet_failure_taxonomy.get('blockedPromotionClaims', []):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy missing verifier-replay failure claim')
    if 'failure taxonomy is a rejection/recovery map, not Classic byte-offset evidence' not in packet_failure_taxonomy.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy missing non-evidence blocker')
    if 'not-promoted' not in packet_failure_taxonomy.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset failure taxonomy must remain non-promoted')
    packet_recovery_plan = data.get('systTemplateOffsetEvidencePacketRecoveryPlanSummary', {})
    if packet_recovery_plan.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-recovery-plan':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet recovery plan')
    if packet_recovery_plan.get('oracleStatus') != 'syst_template_offset_packet_recovery_plan_blocked_pending_real_packet':
        raise ValueError('sourced EV systems manifest template/offset recovery plan changed oracle status')
    if packet_recovery_plan.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketFailureTaxonomySummary' not in packet_recovery_plan.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset recovery plan missing evidence inputs')
    if packet_recovery_plan.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset recovery plan changed schema version')
    if packet_recovery_plan.get('recoveryActionCount') != 5 or 'recover-verifier-replay-missing-or-failed' not in packet_recovery_plan.get('recoveryActionIds', []):
        raise ValueError('sourced EV systems manifest template/offset recovery plan changed recovery actions')
    if packet_recovery_plan.get('failureClassIds') != packet_failure_taxonomy.get('failureClassIds'):
        raise ValueError('sourced EV systems manifest template/offset recovery plan no longer covers all failure classes')
    if 'successful local replay of extractor, focused model validation, and static_topology_source_readiness_scout with captured actual output' not in ' '.join(action.get('requiredNextEvidence', '') for action in packet_recovery_plan.get('recoveryActions', [])):
        raise ValueError('sourced EV systems manifest template/offset recovery plan missing verifier recovery evidence')
    if 'recovery plan is a next-evidence checklist, not Classic byte-offset evidence' not in packet_recovery_plan.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset recovery plan missing non-evidence blocker')
    if 'not-promoted' not in packet_recovery_plan.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset recovery plan must remain non-promoted')
    packet_reentry_guardrail = data.get('systTemplateOffsetEvidencePacketReentryGuardrailSummary', {})
    if packet_reentry_guardrail.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-reentry-guardrail':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet reentry guardrail')
    if packet_reentry_guardrail.get('oracleStatus') != 'syst_template_offset_packet_reentry_blocked_pending_recovered_real_packet':
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail changed oracle status')
    if packet_reentry_guardrail.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketRecoveryPlanSummary' not in packet_reentry_guardrail.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail missing evidence inputs')
    if packet_reentry_guardrail.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail changed schema version')
    if packet_reentry_guardrail.get('failureClassIds') != packet_recovery_plan.get('failureClassIds'):
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail no longer covers all failure classes')
    if packet_reentry_guardrail.get('reentryStepCount') != 4 or 'recovered-packet-contract-matrix-triage-rerun' not in packet_reentry_guardrail.get('reentryStepIds', []):
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail changed reentry steps')
    if 'do not resume from the failure point or skip earlier acceptance gates because a packet was revised' not in packet_reentry_guardrail.get('blockedShortcuts', []):
        raise ValueError('sourced EV systems manifest template/offset reentry guardrail missing shortcut blocker')
    if 'reentry guardrail is a replay path, not Classic byte-offset evidence' not in packet_reentry_guardrail.get('promotionBlockers', []):
    ...
    packet_rollback_rehearsal = data.get('systTemplateOffsetEvidencePacketRollbackRehearsalSummary', {})
    if packet_rollback_rehearsal.get('sourceLabel') != 'repo-reference-backed-syst-template-offset-evidence-packet-rollback-rehearsal':
        raise ValueError('sourced EV systems manifest missing template/offset evidence packet rollback rehearsal')
    if packet_rollback_rehearsal.get('oracleStatus') != 'syst_template_offset_packet_rollback_rehearsal_blocked_pending_actual_reverted_packet_diff':
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal changed oracle status')
    if packet_rollback_rehearsal.get('evidenceInputSummaryCount') != 4 or 'systTemplateOffsetEvidencePacketRollbackReadinessSummary' not in packet_rollback_rehearsal.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal missing evidence inputs')
    if packet_rollback_rehearsal.get('contractSchemaVersion') != 1:
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal changed schema version')
    if packet_rollback_rehearsal.get('rollbackControlIds') != packet_rollback_readiness.get('rollbackControlIds'):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal no longer preserves rollback controls')
    if packet_rollback_rehearsal.get('rehearsalStepCount') != 4 or 'post-revert-verifier-replay-capture' not in packet_rollback_rehearsal.get('rehearsalStepIds', []):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal changed step set')
    if 'python3 tools/run_gameplay_scenarios.py static_topology_source_readiness_scout --pretty' not in packet_rollback_rehearsal.get('requiredVerifierAfterRehearsal', []):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal missing scenario replay verifier')
    if 'do not mark a rollback rehearsal ready from stale pre-revert verifier output' not in packet_rollback_rehearsal.get('blockedShortcuts', []):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal missing stale-output blocker')
    if 'rollback rehearsal is an operational safety checklist, not Classic byte-offset evidence' not in packet_rollback_rehearsal.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal missing non-evidence blocker')
    if 'not-promoted' not in packet_rollback_rehearsal.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest template/offset rollback rehearsal must remain non-promoted')
    ...
    runtime_capture_validation_matrix = data.get('nonTopologySystRuntimeCaptureValidationMatrixSummary', {})
    if runtime_capture_validation_matrix.get('sourceLabel') != 'decoded-resource-backed-non-topology-syst-runtime-capture-validation-matrix':
        raise ValueError('sourced EV systems manifest missing non-topology runtime-capture validation matrix')
    if runtime_capture_validation_matrix.get('oracleStatus') != 'non_topology_syst_capture_validation_blocked_pending_complete_control_packets':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix has unexpected oracle status')
    if runtime_capture_validation_matrix.get('recordCount') != 67 or runtime_capture_validation_matrix.get('evidenceInputSummaryCount') != 4:
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix has unexpected counts')
    if runtime_capture_validation_matrix.get('firstPriorityProbeFamily') != 'government-message-hazard-visibility-disambiguation':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix changed first probe family')
    if runtime_capture_validation_matrix.get('requiredValidatedCapturePacketCount') != 2:
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix changed packet count')
    if runtime_capture_validation_matrix.get('validationRowCount') != 4 or 'positive-negative-control-pair-completeness' not in runtime_capture_validation_matrix.get('validationCheckIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix changed checks')
    if 'runtime-observation-used-as-offset-oracle' not in runtime_capture_validation_matrix.get('failureClassIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix missing offset-oracle failure class')
    if 'do not assign Govt/Message/Asteroids/Interference/VisBit offsets from runtime observation alone' not in runtime_capture_validation_matrix.get('blockedShortcuts', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix missing offset shortcut blocker')
    if 'capture validation matrix is an acceptance checklist, not Classic byte-offset evidence' not in runtime_capture_validation_matrix.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix missing promotion blocker')
    if 'not-promoted' not in runtime_capture_validation_matrix.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture validation matrix must remain non-promoted')
    runtime_capture_rejection_taxonomy = data.get('nonTopologySystRuntimeCaptureRejectionTaxonomySummary', {})
    if runtime_capture_rejection_taxonomy.get('sourceLabel') != 'decoded-resource-backed-non-topology-syst-runtime-capture-rejection-taxonomy':
        raise ValueError('sourced EV systems manifest missing non-topology runtime-capture rejection taxonomy')
    if runtime_capture_rejection_taxonomy.get('oracleStatus') != 'non_topology_syst_capture_rejection_taxonomy_blocked_pending_real_control_packets':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy has unexpected oracle status')
    if runtime_capture_rejection_taxonomy.get('recordCount') != 67 or runtime_capture_rejection_taxonomy.get('evidenceInputSummaryCount') != 4:
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy has unexpected counts')
    if runtime_capture_rejection_taxonomy.get('firstPriorityProbeFamily') != 'government-message-hazard-visibility-disambiguation':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy changed first probe family')
    if 'non-topology-gov-message-hazard-visibility-positive-control' not in runtime_capture_rejection_taxonomy.get('requiredCapturePacketIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy missing positive control packet')
    if runtime_capture_rejection_taxonomy.get('rejectionClassCount') != 4 or 'topology-boundary-contamination' not in runtime_capture_rejection_taxonomy.get('rejectionClassIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy changed classes')
    if 'block-promotion-and-preserve-as-priority-refinement-only' not in runtime_capture_rejection_taxonomy.get('rejectionDispositions', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy missing promotion-block disposition')
    if 'rejection taxonomy is a failure-routing checklist, not Classic byte-offset evidence' not in runtime_capture_rejection_taxonomy.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy missing promotion blocker')
    if 'not-promoted' not in runtime_capture_rejection_taxonomy.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture rejection taxonomy must remain non-promoted')
    runtime_capture_reentry_guardrail = data.get('nonTopologySystRuntimeCaptureReentryGuardrailSummary', {})
    if runtime_capture_reentry_guardrail.get('sourceLabel') != 'decoded-resource-backed-non-topology-syst-runtime-capture-reentry-guardrail':
        raise ValueError('sourced EV systems manifest missing non-topology runtime-capture reentry guardrail')
    if runtime_capture_reentry_guardrail.get('oracleStatus') != 'non_topology_syst_capture_reentry_blocked_pending_corrected_control_packets':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail has unexpected oracle status')
    if runtime_capture_reentry_guardrail.get('recordCount') != 67 or runtime_capture_reentry_guardrail.get('evidenceInputSummaryCount') != 4:
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail has unexpected counts')
    if runtime_capture_reentry_guardrail.get('firstPriorityProbeFamily') != 'government-message-hazard-visibility-disambiguation':
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail changed first probe family')
    if 'non-topology-gov-message-hazard-visibility-positive-control' not in runtime_capture_reentry_guardrail.get('requiredCapturePacketIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail missing positive control packet')
    if runtime_capture_reentry_guardrail.get('reentryStepCount') != 4 or 'topology-boundary-redaction' not in runtime_capture_reentry_guardrail.get('reentryStepIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail changed steps')
    if 'runtime-observation-used-as-offset-oracle' not in runtime_capture_reentry_guardrail.get('rejectionClassIds', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail missing rejection class input')
    if 'reentry guardrail is a replay-path checklist, not Classic byte-offset evidence' not in runtime_capture_reentry_guardrail.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail missing promotion blocker')
    if 'not-promoted' not in runtime_capture_reentry_guardrail.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest non-topology runtime-capture reentry guardrail must remain non-promoted')
    ...
    if runtime_capture_reconciliation.get('recordCount') != 67 or runtime_capture_reconciliation.get('evidenceInputSummaryCount') != 5:
    ...
    record_name_reconciliation = data.get('recordNameRuntimeJoinReconciliationSummary', {})
    if record_name_reconciliation.get('sourceLabel') != 'decoded-resource-and-runtime-backed-record-name-join-reconciliation-plan':
        raise ValueError('sourced EV systems manifest missing record-name runtime join reconciliation plan')
    if record_name_reconciliation.get('oracleStatus') != 'record_name_join_reconciliation_blocked_pending_complete_name_or_runtime_packets':
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation changed oracle status')
    if record_name_reconciliation.get('evidenceInputSummaryCount') != 5 or 'recordNameOracleEvidenceMatrixSummary' not in record_name_reconciliation.get('evidenceInputSummaries', []):
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation missing evidence inputs')
    if record_name_reconciliation.get('exactMappedResourceIds') != [128] or record_name_reconciliation.get('unjoinedRecordCount') != 66:
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation changed mapping boundary')
    if record_name_reconciliation.get('unjoinedStartNeighborResourceIds') != [129, 130, 131]:
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation changed start-neighbor boundary')
    if 'route-label-con4-resource-131' not in record_name_reconciliation.get('requiredRuntimePacketIds', []):
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation missing priority route-label packet')
    if 'post-evidence reconciliation is a gate, not a record-name promotion packet' not in record_name_reconciliation.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation missing promotion blocker')
    if 'not-promoted' not in record_name_reconciliation.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest record-name runtime join reconciliation must remain non-promoted')
    cross_ref = data.get('systRecordNameCandidateCrossReferenceSummary', {})
    if cross_ref.get('sourceLabel') != 'decoded-resource-backed-syst-record-name-candidate-cross-reference':
        raise ValueError('sourced EV systems manifest missing syst record name candidate cross-reference source label')
    if cross_ref.get('oracleStatus') != 'exact_record_name_runtime_topology_mapping_pending':
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference must keep runtime topology pending')
    if cross_ref.get('recordCount') != 67:
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference has unexpected record count')
    if cross_ref.get('exactMatchCount') != 1:
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference must have exactly 1 exact match')
    if cross_ref.get('landingNameCandidateCount') != 72:
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference has unexpected landing-name candidate count')
    alignment = cross_ref.get('alignmentBasis', {})
    if alignment.get('anchorChunkIndex') != 11 or alignment.get('anchorResourceId') != 128 or alignment.get('anchorSystemName') != 'Levo':
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference has unexpected alignment basis')
    if alignment.get('hypothesis') != 'landing-name chunkIndex ordering corresponds to EV Data.rez system resource ID ordering':
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference has unexpected hypothesis')
    if 'only resource 128 to Levo has exact Classic and runtime evidence' not in str(cross_ref.get('promotionBlockers', [])):
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference missing key promotion blocker')
    if 'not-promoted' not in cross_ref.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference must remain non-promoted')
    xref_list = cross_ref.get('crossReference', [])
    if len(xref_list) != 67:
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference has unexpected entry count')
    first_entry = xref_list[0]
    if first_entry.get('resourceId') != 128 or first_entry.get('matchType') != 'exact' or first_entry.get('candidateSystemName') != 'Levo':
        raise ValueError('sourced EV systems manifest syst record name candidate cross-reference first entry must be exact Levo')
    ...
    identity_resolution = data.get('coordinateGapIdentityResolutionSummary', {})
    if identity_resolution.get('sourceLabel') != 'decoded-resource-backed-coordinate-gap-identity-resolution-scout':
        raise ValueError('sourced EV systems manifest missing coordinate gap identity resolution source label')
    if identity_resolution.get('oracleStatus') != 'coordinate_display_units_map_scaling_pending':
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution must keep display units pending')
    if identity_resolution.get('recordCount') != 67:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution has unexpected record count')
    if identity_resolution.get('gapCount') != 21:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution must have 21 gap entries')
    gaps = identity_resolution.get('gapResolutions', [])
    if len(gaps) != 21:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution entry count changed')
    if identity_resolution.get('coLocatedCount', 0) + identity_resolution.get('nearCount', 0) + identity_resolution.get('separatedCount', 0) != 21:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution classification counts do not sum to 21')
    first_gap = gaps[0]
    if first_gap.get('resourceId') != 135:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution first entry resourceId changed')
    if 'classification' not in first_gap or 'classificationHypothesis' not in first_gap:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution missing classification fields')
    if 'coLocatedWith' not in first_gap:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution missing coLocatedWith fields')
    if identity_resolution.get('coordinateSpanX', 0) <= 0 or identity_resolution.get('coordinateSpanY', 0) <= 0:
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution has invalid coordinate spans')
    if 'gap identity hypotheses are spatial proximity signals only' not in str(identity_resolution.get('promotionBlockers', [])):
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution missing identity-hypothesis blocker')
    if 'not-promoted' not in identity_resolution.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest coordinate gap identity resolution must not be promoted')
    deduplication = data.get('coordinateGapResourceDeduplicationSummary', {})
    if deduplication.get('sourceLabel') != 'decoded-resource-backed-coordinate-gap-resource-deduplication-scout':
        raise ValueError('sourced EV systems manifest missing coordinate gap resource deduplication source label')
    if deduplication.get('oracleStatus') != 'coordinate_display_units_map_scaling_pending':
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication must keep display units pending')
    if deduplication.get('totalResourceIds') != 67 or deduplication.get('namedSystemCount') != 46:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication has unexpected system counts')
    if deduplication.get('gapCount') != 21 or deduplication.get('coLocatedDuplicateCount') != 16:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication has unexpected gap/duplicate counts')
    if deduplication.get('nearOrSeparatedCount') != 5:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication has unexpected near/separated count')
    if deduplication.get('simplifiedDistinctSystemCountEstimate') != 51:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication has unexpected simplified count')
    dedup_entries = deduplication.get('deduplication', [])
    if len(dedup_entries) != 16:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication entry count changed')
    first_dedup = dedup_entries[0]
    if first_dedup.get('duplicateResourceId') != 135:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication first entry resourceId changed')
    if first_dedup.get('canonicalSystemName') != 'Master\u2019s Planet':
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication first entry canonical name changed')
    if 'confidence' not in first_dedup or first_dedup['confidence'] != 'spatial-proximity-hypothesis':
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication missing confidence field')
    if 'allCoLocatedNamedCandidates' not in first_dedup:
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication missing candidate field')
    if 'deduplication is spatial proximity scouting only' not in str(deduplication.get('promotionBlockers', [])):
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication missing scouting blocker')
    if 'not-promoted' not in deduplication.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest coordinate gap resource deduplication must not be promoted')
    ...
    runtime_route_label_bridge = data.get('runtimeRouteLabelObservationBridgeGapSummary', {})
    if runtime_route_label_bridge.get('sourceLabel') != 'original-runtime-observed-route-label-bridge-gap':
        raise ValueError('sourced EV systems manifest missing runtime route-label observation bridge gap')
    if runtime_route_label_bridge.get('oracleStatus') != 'runtime_route_labels_observed_but_not_joined_to_decoded_resource_ids':
        raise ValueError('sourced EV systems manifest route-label bridge must keep resource-ID joins blocked')
    observed_labels = [entry.get('label') for entry in runtime_route_label_bridge.get('observedRuntimeLabels', [])]
    if runtime_route_label_bridge.get('observedRuntimeLabelCount') != 4 or not {'Rigel', 'Kathoon'}.issubset(set(observed_labels)):
        raise ValueError('sourced EV systems manifest route-label bridge changed observed-label inventory')
    if runtime_route_label_bridge.get('candidateStartResourceId') != 128 or runtime_route_label_bridge.get('unjoinedStartNeighborResourceIds') != [129, 130, 131]:
        raise ValueError('sourced EV systems manifest route-label bridge changed start-neighbor boundary')
    if 'runtime labels are visible Classic observations but no capture currently ties each label to a decoded resource ID or Con slot' not in runtime_route_label_bridge.get('bridgeBlockers', []):
        raise ValueError('sourced EV systems manifest route-label bridge missing resource-ID join blocker')
    if 'not-promoted' not in runtime_route_label_bridge.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest route-label bridge must not promote named topology')
    route_label_probe_targeting = data.get('runtimeRouteLabelProbeTargetingSummary', {})
    if route_label_probe_targeting.get('sourceLabel') != 'original-runtime-and-decoded-resource-backed-route-label-probe-targeting-matrix':
        raise ValueError('sourced EV systems manifest missing runtime route-label probe targeting matrix')
    if route_label_probe_targeting.get('oracleStatus') != 'route_label_probe_targets_pending_decoded_resource_id_slot_join':
        raise ValueError('sourced EV systems manifest route-label probe targeting must keep resource-ID joins blocked')
    if 'original-runtime-observed' not in route_label_probe_targeting.get('sourceBasis', []) or 'decoded-original-variable' not in route_label_probe_targeting.get('sourceBasis', []):
        raise ValueError('sourced EV systems manifest route-label probe targeting missing source/fidelity basis')
    if route_label_probe_targeting.get('candidateStartResourceId') != 128 or route_label_probe_targeting.get('probeTargetCount') != 3:
        raise ValueError('sourced EV systems manifest route-label probe targeting changed Levo target boundary')
    if route_label_probe_targeting.get('unjoinedStartNeighborResourceIds') != [129, 130, 131]:
        raise ValueError('sourced EV systems manifest route-label probe targeting changed unjoined start neighbors')
    if 'probe targets are worklist entries, not decoded resource-ID/name joins' not in route_label_probe_targeting.get('promotionBlockers', []):
    ...
    if runtime_gate.get('evidenceInputSummaryCount') != 6:
        raise ValueError('sourced EV systems manifest runtime universe replacement gate changed evidence input count')
    for expected_input in [
        ...
        if expected_input not in runtime_gate.get('evidenceInputSummaries', []):
            raise ValueError('sourced EV systems manifest runtime universe replacement gate missing reconciliation evidence input')
    if 'syst_template_offset_oracle_gap' not in gate_names:
        raise ValueError('sourced EV systems manifest runtime universe replacement gate missing template/offset oracle blocker')
    if 'record-name joins closed by complete source/name table or validated packet-level target evidence' not in runtime_gate.get('gateClosureChecklist', []):
        raise ValueError('sourced EV systems manifest runtime universe replacement gate missing closure checklist')
    if 'syst template/offset gap closed by Classic-specific TMPL/ResEdit/source evidence before assigning unresolved byte offsets or Con6-Con16 split placement' not in runtime_gate.get('gateClosureChecklist', []):
        raise ValueError('sourced EV systems manifest runtime universe replacement gate missing template/offset closure checklist')
    ...
    if 'do not convert Resource Bible syst prose into byte offsets or Con6-Con16 split placement without a Classic-specific template/source oracle' not in runtime_gate.get('promotionBlockers', []):
        raise ValueError('sourced EV systems manifest runtime universe replacement gate missing template/offset fidelity blocker')
    ...
    display_calibration_gate = data.get('coordinateDisplayCalibrationGateSummary', {})
    if display_calibration_gate.get('sourceLabel') != 'decoded-resource-backed-coordinate-display-calibration-gate':
        raise ValueError('sourced EV systems manifest missing coordinate display calibration gate source label')
    if display_calibration_gate.get('oracleStatus') != 'coordinate_display_calibration_blocked_pending_classic_map_evidence':
        raise ValueError('sourced EV systems manifest coordinate display calibration gate should keep Classic map calibration blocked')
    if display_calibration_gate.get('recordCount') != 67 or display_calibration_gate.get('candidateInputSummaryCount') != 8:
        raise ValueError('sourced EV systems manifest coordinate display calibration gate has unexpected counts')
    if 'coordinateDisplayScaleInterpretationSummary' not in display_calibration_gate.get('candidateInputSummaries', []):
        raise ValueError('sourced EV systems manifest coordinate display calibration gate missing scale interpretation input')
    if 'display pixel scale or accepted projection surrogate' not in display_calibration_gate.get('calibrationRequiredClaims', []):
        raise ValueError('sourced EV systems manifest coordinate display calibration gate missing pixel/projection required claim')
    if 'blocked' not in display_calibration_gate.get('promotionReadinessStatus', ''):
        raise ValueError('sourced EV systems manifest coordinate display calibration gate must stay blocked')
    if 'candidate coordinate families disagree on scale/aspect interpretation' not in display_calibration_gate.get('promotionBlockers', []):
    ...
    display_runtime_capture_validation = data.get('coordinateDisplayRuntimeCaptureValidationMatrixSummary', {})
    if display_runtime_capture_validation.get('sourceLabel') != 'original-runtime-coordinate-display-capture-validation-matrix':
        raise ValueError('sourced EV systems manifest missing coordinate display runtime capture validation matrix')
    if display_runtime_capture_validation.get('oracleStatus') != 'coordinate_display_capture_validation_blocked_pending_real_map_packets':
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation has unexpected oracle status')
    if display_runtime_capture_validation.get('recordCount') != 67 or display_runtime_capture_validation.get('evidenceInputSummaryCount') != 4:
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation has unexpected counts')
    if display_runtime_capture_validation.get('requiredCapturePacketTargetResourceIds') != [129, 130, 131]:
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation changed target resources')
    if display_runtime_capture_validation.get('validationCheckCount') != 5:
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation changed check count')
    if 'map-pixel-observation-completeness' not in display_runtime_capture_validation.get('validationCheckIds', []):
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation missing pixel observation check')
    if display_runtime_capture_validation.get('failureClassCount') != 4:
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation changed failure class count')
    if 'promotion-boundary-contamination' not in display_runtime_capture_validation.get('failureClassIds', []):
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation missing promotion-boundary failure class')
    if display_runtime_capture_validation.get('minimumAcceptedPacketCountBeforeScaleReview') != 2:
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation changed minimum packet count')
    if 'not-promoted' not in display_runtime_capture_validation.get('promotionStatus', ''):
        raise ValueError('sourced EV systems manifest coordinate display runtime capture validation must remain non-promoted')
